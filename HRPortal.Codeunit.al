codeunit 50150 HRPortal
{
    ...
    [ServiceEnabled]
    procedure PrintPayslip(employeeNo: Code[20]; Period: Date)BaseImage: Text begin
        Clear(JsObject);
        CLEARLASTERROR();
    ...
    [ServiceEnabled]
    procedure PrintP9(employeeNo: Code[20]; Year: Integer)BaseImage: Text begin
        Clear(JsObject);
        CLEARLASTERROR();
    ...
    [ServiceEnabled]
    procedure PrintEmloyeeStatement(employeeNo: Code[20])BaseImage: Text begin
        Clear(JsObject);
        CLEARLASTERROR();
    ...
    [ServiceEnabled]
    procedure fnGetEmployeeDetails(employeeNo: Code[20])RtnV: Text begin
        Clear(JsObject);
        CLEARLASTERROR();
    ...
    begin
        if leaveNo = '' then begin
            ...
            begin
                ...
                if reliever <> '' then leaveApplications."Leave Reliever":=true;
    ...
    begin
        ...
        if payments.findset(true)then begin
            ...
            if imprestLines.FindLast()then LineNo:=imprestLines."Line No" + 10000
            else
                LineNo:=10000;
            imprestLines.init;
    ...
    begin
        ...
        if payments.findset(true)then begin
            ...
            if paymentLines.FindLast()then LineNo:=paymentLines."Line No" + 10000
            else
                LineNo:=10000;
            paymentLines.init;
    ...
    begin
        ...
        if purchase.findset(true)then begin
            ...
            if purchaseLines.FindLast()then LineNo:=purchaseLines."Line No." + 10000
            else
                LineNo:=10000;
            purchaseLines.init;
    ...
    begin
        ...
        if calendar.findset(true)then begin
            Clear(JsObject);
            Clear(JsArray);
            repeat hoursPerDay:=0;
                currDate:=Format(Calendar."Period Start");
                dateName:=TimeSheetMgt.FormatDate(Calendar."Period Start", 1);
                if CalendarMgmt.IsNonworkingDay(Calendar."Period Start", CustomCalendarChange)then hoursPerDay:=0
                else
                    Clear(DateJsObject);
    ...
    [ServiceEnabled]
    procedure HandleTimeEntryRequest(jsonText: Text)RtnV: Text var
 ...
 begin
    ...
    begin
        ...
        if not JsArray.ReadFrom(workHoursJson)then Error('Invalid JSON format.');
    ...
    begin
        ...
        if timesheetHeader.findset(true)then begin
            ...
            if timeSheetLine.findset(true)then myTimesheetLines+=timeSheetLine.Count;
            timeSheetLine.Init;
    ...
    begin
        ...
        if tbl_timesheetHeader.findset(true)then begin
            ...
            if tbl_timeSheetLine.findset(true)then myTimesheetLines+=tbl_timeSheetLine.Count;
    ...
    [ServiceEnabled]
    procedure fnGetPayments(salaryNumber: Code[30]; employeeNumber: Code[30]; documentType: Text; status: Text)data: Text var
    ...
    begin
        if documentType = 'Petty Cash Surrender' then begin
            ...
            if tbl_payments.FindSet(true)then begin
                repeat data:=data + tbl_payments."No." + '*' + Format(tbl_payments.Date) + '*' + tbl_payments."Payment Narration" + '*' + Format(tbl_payments.Status) + '*' + Format(tbl_payments."Actual Petty Cash Amount Spent") + '*' + Format(tbl_payments.Posted) + '*' + Format(tbl_payments."Total Amount") + '::::';
        ...
        if documentType = 'Staff Claim' then begin
            ...
            if tbl_payments.FindSet(true)then begin
                repeat data+=tbl_payments."No." + '*' + Format(tbl_payments.Date) + '*' + tbl_payments."Payment Narration" + '*' + Format(tbl_payments.Status) + '*' + Format(tbl_payments."Actual Petty Cash Amount Spent") + '*' + tbl_payments."Payment Narration" + '::::';
                until tbl_payments.Next = 0;
            end;
            exit(data);
        end;
        ...
        if documentType = 'Surrender' then begin
            ...
            if tbl_payments.FindSet(true)then begin
                repeat tbl_payments.CalcFields("Imprest Amount", "Actual Amount Spent");
    ...
    procedure addFileLinks(recordType: Text[100]; recordNo: Code[50]; filename: Text; fileLink: Text; extensionType: Text)status: Text var
    ...
    begin
        ...
        else if recordType = 'Imprest Surrender' then begin
                status:=fnConvertFile(fileLink, extensionType, 57000, filename, recordNo, 2);
            end
            else if recordType = 'Leave' then begin
                    status:=fnConvertFile(fileLink, extensionType, 69205, filename, recordNo, 3);
                end
                else if recordType = 'Imprest Requisition' then begin
                        status:=fnConvertFile(fileLink, extensionType, 57000, filename, recordNo, 3);
                    end
                    else if recordType = 'Claim' then begin
                            status:=fnConvertFile(fileLink, extensionType, 57000, filename, recordNo, 3);
                        end
                        else if recordType = 'Store Requisition' then begin
                                status:=fnConvertFile(fileLink, extensionType, 38, filename, recordNo, 11);
                            end end;
    procedure fnConvertFile(fileLink: Text; extensionType: Text; tableId: Integer; fileName: Text; recordNo: Code[30]; documentType: Integer)status: Text var
        tbl_documentAttachments: Record "Document Attachment";
        importFile: File;
    ...
    begin
        ...
        else if(extensionType.ToLower() = 'docx')then begin
                tbl_documentAttachments."File Type":=tbl_documentAttachments."File Type"::Word;
            end
            else if(extensionType.ToLower() = 'xlsx')then begin
                    tbl_documentAttachments."File Type":=tbl_documentAttachments."File Type"::Excel;
                end
                else if((extensionType.ToLower() = 'png') OR (extensionType.ToLower() = 'jpeg') OR (extensionType.ToLower() = 'jpg'))then begin
    ...
    procedure fnConvertFile1(fileContent: Text; extensionType: Text; tableId: Integer; fileName: Text; recordNo: Code[30]; documentType: Integer)status: Text var
        tbl_documentAttachments: Record "Document Attachment";
        fileInstream: InStream;
        fileId: Guid;
    ...
    procedure fnDownloadFile(fileName: Text; recordNo: Code[30])status: Text var
        tbl_documentAttachments: Record "Document Attachment";
        exportFile: File;
    ...
    begin
        ...
        if leaveApplications.FindFirst()then begin
            ...
            //send approval
            If ApprovalMgt.CheckLeaveRecallWorkflowEnabled(VarVariant)then ApprovalMgt.OnSendLeaveRequestApproval(VarVariant);
    ...
    begin
        ...
        if purchasesRec.FindFirst then begin
            ...
            if ApprovalMgt.CheckReqWorkflowEnabled(purchasesRec)then ApprovalMgt.OnSendReqRequestforApproval(purchasesRec);
            Commit;
        end;
        JsObject.Add('Error', 'FALSE');
    end;
    ...
    begin
        ...
        if trainingRec.FindFirst then begin
            if ApprovalMgt.CheckTrainingRequestWorkflowEnabled(trainingRec)then ApprovalMgt.OnSendTrainingRequestforApproval(trainingRec);
            Commit;
        end;
        JsObject.Add('Error', 'FALSE');
    end;
    ...
    procedure fnApproveRequest(docNo: Code[20]; username: Text)data: Text var
    ...
    begin
        ...
        if approvalEntry.findset(true)then begin
            ...
            begin
                data+='danger*Sorry, you could not approve the request. Kindly try again.' end;
        end
        else
        begin
    ...
    [ServiceEnabled]
    procedure FnUploadAttachedDocument(docNo: Code[50]; fileName: Text; fileExt: Text; attachment: Text; tableID: Integer)returnV: Text begin
        Clear(JsObject);
        CLEARLASTERROR();
    ...
    [ServiceEnabled]
    procedure FnDeleteDocumentAttachment(docNo: Code[20]; tableID: Integer; docID: Integer)returnV: Text var
        DocAttachment: Record "Document Attachment";
    begin
        Clear(JsObject);
        CLEARLASTERROR();
    ...
    [ServiceEnabled]
    procedure FnGetDocumentAttachment(tableId: Integer; no: Code[20]; recID: Integer)BaseImage: Text begin
        Clear(JsObject);
        CLEARLASTERROR();
