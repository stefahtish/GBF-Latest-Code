report 50106 Imprest
{
    DefaultLayout = RDLC;
    RDLCLayout = './Imprest.rdl';
    WordLayout = './Imprest.docx';
    ApplicationArea = All;
    ...
    dataset
    {
        dataitem(Payments; Payments)
        {
            ...
            column(CheckOtherEmployeeImpress; CheckOtherEmployeeImpress)
            {
            }
            ...
            column(Approved5; GetUserName(Approver[5]))
            {
            }
            ...
            dataitem(ExtPayments; "Ext Payment Lines")
            {
                DataItemLink = No = FIELD("No.");
                DataItemTableView = SORTING(No, "Line No");

                column(ExtNo_ImprestLines; ExtPayments.No)
                {
                }
                column(ExtLineNo_ImprestLines; ExtPayments."Line No")
                {
                }
                column(ExtAccountType_ImprestLines; ExtPayments."Account Type")
                {
                }
                column(ExtAccountNo_ImprestLines; ExtPayments."Account No")
                {
                }
                column(ExtAccountName_ImprestLines; ExtPayments."Account Name")
                {
                }
                column(ExtDescription_ImprestLines; ExtPayments.Description)
                {
                }
                column(ExtAmount_ImprestLines; ExtPayments.Amount)
                {
                }
                column(ExtAppliestoDocNo_ImprestLines; ExtPayments."Applies-to Doc. No.")
                {
                }
                column(ExtGlobalDimension1Code_ImprestLines; ExtPayments."Shortcut Dimension 1 Code")
                {
                }
                column(ExtGlobalDimension2Code_ImprestLines; ExtPayments."Shortcut Dimension 2 Code")
                {
                }
                column(ExtActualSpent_ImprestLines; ExtPayments."Actual Spent")
                {
                }
                column(ExtRemainingAmount_ImprestLines; ExtPayments."Remaining Amount")
                {
                }
                column(ExtCommitted_ImprestLines; ExtPayments.Committed)
                {
                }
                column(ExtAmountLCY_ImprestLines; ExtPayments."Actual Spent (LCY)")
                {
                }
                column(ExtPurpose_ImprestLines; ExtPayments.Purpose)
                {
                }
                column(ExtExpenditureType_PaymentLines; ExtPayments."Expenditure Type")
                {
                }
                column(ExtDimValueName_1; DimValueName[1])
                {
                }
                column(ExtDimValueName_2; DimValueName[2])
                {
                }
                column(ExtDimValueName_3; DimValueName[3])
                {
                }
                column(ExtDimValueName_4; DimValueName[4])
                {
                }
                column(ExtDimValueName_5; DimValueName[5])
                {
                }
                column(ExtDimValueName_6; DimValueName[6])
                {
                }
                column(ExtDimValueName_7; DimValueName[7])
                {
                }
                column(ExtDimValueName_8; DimValueName[8])
                {
                }
                column(ExtMultiDonor_Fund; MultiDonor_Fund)
                {
                }
                column(ExtMultiDonor_Theme; MultiDonor_Theme)
                {
                }
                column(ExtDimValueCode_1; DimValueCode[1])
                {
                }
                column(Name; Format("ID Number") + ' - ' + Name)
                {
                }
                ...
                begin
                    //Dimension Values
                    DimSetEntry.Reset;
                    DimSetEntry.SetRange("Dimension Set ID", ExtPayments."Dimension Set ID");
                    DimSetEntry.SetRange("Dimension Code", GLSetup."Global Dimension 1 Code");
                    if DimSetEntry.FindFirst then begin
                        DimSetEntry.CalcFields("Dimension Value Name");
                        DimValueCode[1] := DimSetEntry."Dimension Value Code";
    ...
    local procedure CheckOtherEmployeeImpress(): Boolean
    var
        ExtPayments: Record "Ext Payment Lines";
    begin
        ExtPayments.Reset();
        ExtPayments.SetFilter(No, '%1', Payments."No.");
        if ExtPayments.FindFirst() then exit(true);
    end;
