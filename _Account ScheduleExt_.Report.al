report 50167 "Account ScheduleExt"
{
    DefaultLayout = RDLC;
    RDLCLayout = './CustomBaseApp/Reports/Report 51519068-ReportExtAccSchedule.rdlc';
    ...
    dataset
    {
        dataitem(AccScheduleName; "Acc. Schedule Name")
        {
            ...
            dataitem(Heading; "Integer")
            {
                ...
                dataitem("Acc. Schedule Line"; "Acc. Schedule Line")
                {
                    ...
                    dataitem("Column Layout"; "Column Layout")
                    {
                        ...
                        begin
                            if Show = Show::Never then CurrReport.Skip();
                            Header:="Column Header";
                            RoundingHeader:='';
                            if "Rounding Factor" in["Rounding Factor"::"1000", "Rounding Factor"::"1000000"]then case "Rounding Factor" of "Rounding Factor"::"1000": RoundingHeader:=Text000;
                                "Rounding Factor"::"1000000": RoundingHeader:=Text001;
                                end;
                            ColumnValuesAsText:=CalcColumnValueAsText("Acc. Schedule Line", "Column Layout");
                            ColumnValuesArrayIndex+=1;
                            if ColumnValuesArrayIndex <= ArrayLen(ColumnValuesArrayText)then ColumnValuesArrayText[ColumnValuesArrayIndex]:=ColumnValuesAsText;
                            if(ColumnValuesAsText <> '') or (("Acc. Schedule Line".Show = "Acc. Schedule Line".Show::Yes) and not SkipEmptyLines)then LineSkipped:=false;
                            if(ColumnValuesAsText = '') and ("Acc. Schedule Line"."Totaling Type" in["Acc. Schedule Line"."Totaling Type"::"Posting Accounts", "Acc. Schedule Line"."Totaling Type"::Formula])then ColumnValuesAsText:='0.00';
                        end;
                    ...
                    trigger OnAfterGetRecord()
                    begin
                        if(Show = Show::No) or not ShowLine(Bold, Italic)then CurrReport.Skip();
                ...
                begin
                    ...
                    if ColumnLayout.FindSet then repeat i+=1;
            ...
            begin
                ...
                if UseAmtsInAddCurr then HeaderText:=StrSubstNo(Text003, GLSetup."Additional Reporting Currency")
                else if GLSetup."LCY Code" <> '' then HeaderText:=StrSubstNo(Text003, GLSetup."LCY Code")
                    else
                        HeaderText:='';
            end;
            trigger OnPreDataItem()
    ...
    requestpage
    {
                ...
                group(Options)
                {
                    ...
                    group("Layout")
                    {
                        ...
                        field(ColumnLayoutNames; ColumnLayoutName)
                        {
                            ...
                            trigger OnLookup(var Text: Text): Boolean begin
                                if not AccSchedManagement.LookupColumnName(ColumnLayoutName, Text)then exit(false);
                            ...
                            trigger OnValidate()
                            begin
                                if ColumnLayoutName = '' then Error(Text006);
        ...
        begin
            GLSetup.Get();
            TransferValues;
            if AccSchedName <> '' then if(ColumnLayoutName = '') or not AccSchedNameEditable then ValidateAccSchedName;
            SetBudgetFilterEnable;
        end;
    }
    labels
    ...
    PadChar: Char;
    PadString: Text;
    local procedure CalcColumnValueAsText(var AccScheduleLine: Record "Acc. Schedule Line"; var ColumnLayout: Record "Column Layout"): Text[30]var
        ColumnValuesAsText: Text[30];
    begin
        ColumnValuesAsText:='';
        ColumnValuesDisplayed:=AccSchedManagement.CalcCell(AccScheduleLine, ColumnLayout, UseAmtsInAddCurr);
        //Check Budget Entries Eddie BPIT
        if("Column Layout"."Ledger Entry Type" = "Column Layout"."Ledger Entry Type"::"Budget Entries") and ("Acc. Schedule Line"."Show Opposite Sign")then begin
            if ColumnValuesDisplayed < 0 then ColumnValuesDisplayed:=-ColumnValuesDisplayed;
        end;
        if AccSchedManagement.GetDivisionError then begin
            if ShowError in[ShowError::"Division by Zero", ShowError::Both]then ColumnValuesAsText:=Text002;
        end
        else if AccSchedManagement.GetPeriodError then begin
                if ShowError in[ShowError::"Period Error", ShowError::Both]then ColumnValuesAsText:=Text004;
            end
            else
            begin
                ColumnValuesAsText:=AccSchedManagement.FormatCellAsText(ColumnLayout, ColumnValuesDisplayed, UseAmtsInAddCurr);
                if AccScheduleLine."Totaling Type" = AccScheduleLine."Totaling Type"::Formula then case AccScheduleLine.Show of AccScheduleLine.Show::"When Positive Balance": if ColumnValuesDisplayed < 0 then ColumnValuesAsText:='';
                    AccScheduleLine.Show::"When Negative Balance": if ColumnValuesDisplayed > 0 then ColumnValuesAsText:='';
                    AccScheduleLine.Show::"If Any Column Not Zero": if ColumnValuesDisplayed = 0 then ColumnValuesAsText:='';
                    end;
            end;
    ...
    begin
        ...
        if "Acc. Schedule Line".GetFilter("Date Filter") <> '' then EndDate:="Acc. Schedule Line".GetRangeMax("Date Filter");
        ...
        if StartDateEnabled then PeriodText:=PeriodTextCaptionLbl + ': ' + "Acc. Schedule Line".GetFilter("Date Filter")
        else
            PeriodText:=PeriodEndingTextCaptionLbl + ' ' + Format(EndDate);
        if ShowRowNo then RowNoCaption:="Acc. Schedule Line".FieldCaption("Row No.");
        ColumnLayout.SetRange("Column Layout Name", ColumnLayoutName);
    ...
    procedure ShowLine(Bold: Boolean; Italic: Boolean): Boolean begin
        if "Acc. Schedule Line"."Totaling Type" = "Acc. Schedule Line"."Totaling Type"::"Set Base For Percent" then exit(false);
        if "Acc. Schedule Line".Show = "Acc. Schedule Line".Show::No then exit(false);
        if "Acc. Schedule Line".Bold <> Bold then exit(false);
        if "Acc. Schedule Line".Italic <> Italic then exit(false);
        exit(true);
    end;
    local procedure FormLookUpDimFilter(Dim: Code[20]; var Text: Text[1024]): Boolean var
    ...
    begin
        ...
        exit(false)end;
    local procedure FormGetCaptionClass(DimNo: Integer): Text[250]begin
        case DimNo of 1: begin
            if AnalysisView."Dimension 1 Code" <> '' then exit('1,6,' + AnalysisView."Dimension 1 Code");
            exit(StrSubstNo(Text005, DimNo));
        end;
        2: begin
            if AnalysisView."Dimension 2 Code" <> '' then exit('1,6,' + AnalysisView."Dimension 2 Code");
            exit(StrSubstNo(Text005, DimNo));
        end;
        3: begin
            if AnalysisView."Dimension 3 Code" <> '' then exit('1,6,' + AnalysisView."Dimension 3 Code");
            exit(StrSubstNo(Text005, DimNo));
        end;
        4: begin
            if AnalysisView."Dimension 4 Code" <> '' then exit('1,6,' + AnalysisView."Dimension 4 Code");
            exit(StrSubstNo(Text005, DimNo));
        end;
        end;
    end;
    ...
    begin
        if GLBudgetName <> '' then GLBudgetFilter:=GLBudgetName;
        GLSetup.Get();
        UseAmtsInAddCurrVisible:=GLSetup."Additional Reporting Currency" <> '';
        BusinessUnitFilterVisible:=not BusinessUnit.IsEmpty;
        if not UseAmtsInAddCurrVisible then UseAmtsInAddCurr:=false;
        if AccSchedNameHidden <> '' then AccSchedName:=AccSchedNameHidden;
        if ColumnLayoutNameHidden <> '' then ColumnLayoutName:=ColumnLayoutNameHidden;
        if DateFilterHidden <> '' then DateFilter:=DateFilterHidden;
        if GLBudgetFilterHidden <> '' then GLBudgetFilter:=GLBudgetFilterHidden;
        if CostBudgetFilterHidden <> '' then CostBudgetFilter:=CostBudgetFilterHidden;
        if BusinessUnitFilterHidden <> '' then BusinessUnitFilter:=BusinessUnitFilterHidden;
        if Dim1FilterHidden <> '' then Dim1Filter:=Dim1FilterHidden;
        if Dim2FilterHidden <> '' then Dim2Filter:=Dim2FilterHidden;
        if Dim3FilterHidden <> '' then Dim3Filter:=Dim3FilterHidden;
        if Dim4FilterHidden <> '' then Dim4Filter:=Dim4FilterHidden;
        if AccSchedName <> '' then if not AccScheduleName.Get(AccSchedName)then AccSchedName:='';
        if AccSchedName = '' then if AccScheduleName.FindFirst then AccSchedName:=AccScheduleName.Name;
        if not ColumnLayoutName2.Get(ColumnLayoutName)then ColumnLayoutName:=AccScheduleName."Default Column Layout";
        if AccScheduleName."Analysis View Name" <> '' then AnalysisView.Get(AccScheduleName."Analysis View Name")
        else
        begin
            AnalysisView."Dimension 1 Code":=GLSetup."Global Dimension 1 Code";
            AnalysisView."Dimension 2 Code":=GLSetup."Global Dimension 2 Code";
    ...
    begin
        ...
        if ColumnLayoutName = '' then if AccScheduleName.Get(AccSchedName)then ColumnLayoutName:=AccScheduleName."Default Column Layout";
    end;
    ...
    begin
        ...
        if ColumnLayoutName = '' then exit;
        if not AccSchedNameEditable then exit;
        ColumnLayout.SetRange("Column Layout Name", ColumnLayoutName);
        ...
        if not BudgetFilterEnable then GLBudgetFilter:='';
        ...
        if not StartDateEnabled then StartDate:=0D;
    end;
    local procedure ValidateStartEndDate()
    begin
        if(StartDate = 0D) and (EndDate = 0D)then ValidateDateFilter('')
        else
    ...
    begin
        ...
        if AccScheduleName."Default Column Layout" <> '' then ColumnLayoutName:=AccScheduleName."Default Column Layout";
        if AccScheduleName."Analysis View Name" <> '' then AnalysisView.Get(AccScheduleName."Analysis View Name")
        else
        begin
            Clear(AnalysisView);
