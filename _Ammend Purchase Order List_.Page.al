page 50772 "Ammend Purchase Order List"
{
    Caption = 'Ammend Purchase Order List';
    ApplicationArea = Basic, Suite;
    CardPageID = "Ammend Purchase Orders";
    ...
    SourceTableView = WHERE("Document Type"=CONST(Order), "OrderType"=filter(Ammend));
    UsageCategory = Lists;
        ...
        area(navigation)
        {
            group("O&rder")
            {
                ...
                action(Approvals)
                {
                    ...
                    begin
                    //eddie  WorkflowsEntriesBuffer.AddWorkflowWebhookEntry(Rec.RecordId, DATABASE::"Purchase Header", rec."Document Type".AsInteger(), Rec."No.");
                    end;
                }
                action("Co&mments")
                {
    ...
    begin
        ...
        repeat NextRecNotFound:=Rec.Next <= 0;
            if ShowHeader then exit(true);
    ...
    begin
        repeat NewStepCount:=Rec.Next(Steps);
    ...
    local procedure ShowHeader(): Boolean var
