page 50207 "Apply Vendor Entries Custom"
{
    ...
    begin
        ...
        IF (CalcType = CalcType::Payments) AND (ApplyingVendLedgEntry."Posting Date" < PaymentRec.Date) THEN ERROR(EarlierPostingDateErr, ApplyingVendLedgEntry."Document Type", ApplyingVendLedgEntry."Document No.", PaymentLine."Applies-to Doc Type", PaymentLine.No);
                        ...
                        ApplnType::"Applies-to Doc. No.":
                            begin
                                ...
                                if AppliedVendLedgEntry."Amount to Apply" <> 0 then
                                    ...
                                    AppliedAmount := Round(AppliedVendLedgEntry."Remaining Amount", AmountRoundingPrecision);
                                if PaymentToleranceMgtExt.CheckCalcPmtDiscPaymentsVend(PaymentLine, AppliedVendLedgEntry, 0, false) and ((Abs(PaymentLine.Amount) + ApplnRoundingPrecision >= Abs(AppliedAmount - AppliedVendLedgEntry."Remaining Pmt. Disc. Possible")) or (GenJnlLine.Amount = 0)) then PmtDiscAmount := AppliedVendLedgEntry."Remaining Pmt. Disc. Possible";
                                if not DifferentCurrenciesInAppln then DifferentCurrenciesInAppln := ApplnCurrencyCode <> AppliedVendLedgEntry."Currency Code";
                                CheckRounding;
                            end;
                        ApplnType::"Applies-to ID":
    ...
    begin
        if CalcType = CalcType::Direct then begin
            if ApplyingVendLedgEntry."Entry No." <> 0 then begin
                ...
                if PreviewMode then
                    VendEntryApplyPostedEntries.PreviewApply(Rec, appl)
                else
                    Applied := VendEntryApplyPostedEntries.Apply(Rec, appl);
