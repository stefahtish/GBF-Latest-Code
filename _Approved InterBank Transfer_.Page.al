page 50158 "Approved InterBank Transfer"
{
    ...
    SourceTableView = WHERE("Payment Type" = CONST("Bank Transfer"), Status = CONST(Released), Posted = CONST(false));
    ApplicationArea = All;
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                {
                    Caption = 'Source Bank Amount KSH';
                }
                field("Petty Cash Amount (LCY)"; Rec."Petty Cash Amount (LCY)")
                {
                    Caption = 'Total Receiving Amount (KSH)';
                }
