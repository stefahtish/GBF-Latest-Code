page 50719 "Complaint Interaction Card"
{
                ...
                action(Acknowledge)
                {
                    ...
                    begin
                        if confirm('Are you sure?', false) then begin
                            ...
                            SenderName := CompanyName;
                            Rec.CalcFields("Problem Reported");
                            AcknowledgeBody := 'The complaint: ' + ProblemNotesText + ' has been received and is being acted upon';
                            Subject := Format(Rec."Interaction Type");
                            SMTP.Create(Receipients, Subject, '', true);
                            SMTP.AppendtoBody(AcknowledgeBody);
                            Email.Send(SMTP);
                            //Notify MD
                            AcknowledgeBody := 'The complaint: ' + ProblemNotesText + 'by' + Rec."Client Name" + 'has been received and is being acted upon';
                            Clear(Receipients);
