report 50297 "Coop loan I Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = './CooploanIReport.rdl';
    ApplicationArea = All;
    ...
    dataset
    {
        dataitem("Payroll PeriodX"; "Payroll PeriodX")
        {
            ...
            dataitem(DeductionsX; DeductionsX)
            {
                ...
                DataItemTableView = SORTING(Code) where("Loan Type" = filter("Coop Loan I"));
                ...
                dataitem("Assignment Matrix-X"; "Assignment Matrix-X")
                {
                    ...
                    column(CoopSharesAmt; CoopSharesAmt)
                    {
                    }
                    column(CoopSharesBal; CoopSharesBal)
                    {
                    }
                    ...
                    begin
                        ...
                        Deduct.Reset();
                        Deduct.SetRange(Code, DeductionsX."Share Contributions Code");
                        if Deduct.FindFirst() then begin
                            AssignMatrix.Reset();
                            ...
                            if AssignMatrix.FindFirst() then CoopSharesAmt := AssignMatrix.Amount;
                            DeductionBalances.Reset;
                            DeductionBalances.SetRange("Deduction Code", Deduct.Code);
                            DeductionBalances.SetRange("Employee No", "Assignment Matrix-X"."Employee No");
                            DeductionBalances.Setfilter(Date, '<=%1', "Payroll Period");
                            if DeductionBalances.Find('-') then begin
                                ...
                                case Deduct."Balance Type" of
                                    Deduct."Balance Type"::Increasing:
                                        begin
                                            if Deduct."Exclude Employer Balance" then
                                                CoopSharesBal := (DeductionBalances.Amount + Abs(Deduct."Total Amount") + Abs(Deduct."Share Top Up"))
                                            else
                                                CoopSharesBal := (DeductionBalances.Amount + Abs(Deduct."Total Amount") + Abs(Deduct."Total Amount Employer") + Abs(Deduct."Share Top Up"));
                                        end;
                                    Deduct."Balance Type"::Decreasing:
                                        begin
                                            if Deduct."Exclude Employer Balance" then
                                                CoopSharesBal := DeductionBalances.Amount + Deduct."Total Amount" + Abs(Deduct."Share Top Up")
                                            else
                                                CoopSharesBal := DeductionBalances.Amount + Deduct."Total Amount" + Deduct."Total Amount Employer" + Abs(Deduct."Share Top Up");
                                        end;
                                end;
                            end;
                        end;
    ...
    var
        ...
        CoopSharesAmt: Decimal;
        CoopSharesBal: Decimal;
