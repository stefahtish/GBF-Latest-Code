report 50298 "Coop loan II Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = './CooploanIIReport.rdl';
    ApplicationArea = All;
    ...
    dataset
    {
        dataitem("Payroll PeriodX"; "Payroll PeriodX")
        {
            ...
            dataitem(DeductionsX; DeductionsX)
            {
                ...
                DataItemTableView = SORTING(Code) where("Loan Type" = filter("Coop Loan II"));
                ...
                dataitem("Assignment Matrix-X"; "Assignment Matrix-X")
                {
                    ...
                    column(BenevolentAmt; BenevolentAmt)
                    {
                    }
                    column(BenevolentBal; BenevolentBal)
                    {
                    }
                    ...
                    begin
                        ...
                        //Get Opening Balance and current deduction for sacco contribution
                        Deduct.Reset();
                        Deduct.SetRange(Code, DeductionsX."Benevolent Contributions Code");
                        if Deduct.FindFirst() then begin
                            AssignMatrix.Reset();
                            ...
                            if AssignMatrix.FindFirst() then BenevolentAmt := AssignMatrix.Amount;
                            DeductionBalances.Reset;
                            DeductionBalances.SetRange("Deduction Code", Deduct.Code);
                            DeductionBalances.SetRange("Employee No", "Assignment Matrix-X"."Employee No");
                            DeductionBalances.Setfilter(Date, '<=%1', "Payroll Period");
                            if DeductionBalances.Find('-') then begin
                                Deduct.SetFilter("Employee Filter", '=%1', "Employee No");
                                Deduct.SetRange("Pay Period Filter", DeductionBalances.Date, "Payroll Period");
                                Deduct.CalcFields(Deduct."Total Amount", Deduct."Total Amount Employer", Deduct."Share Top Up");
                                case Deduct."Balance Type" of
                                    Deduct."Balance Type"::Increasing:
                                        begin
                                            if Deduct."Exclude Employer Balance" then
                                                BenevolentBal := (DeductionBalances.Amount + Abs(Deduct."Total Amount") + Abs(Deduct."Share Top Up"))
                                            else
                                                BenevolentBal := (DeductionBalances.Amount + Abs(Deduct."Total Amount") + Abs(Deduct."Total Amount Employer") + Abs(Deduct."Share Top Up"));
                                        end;
                                    Deduct."Balance Type"::Decreasing:
                                        begin
                                            if Deduct."Exclude Employer Balance" then
                                                BenevolentBal := DeductionBalances.Amount + Deduct."Total Amount" + Abs(Deduct."Share Top Up")
                                            else
                                                BenevolentBal := DeductionBalances.Amount + Deduct."Total Amount" + Deduct."Total Amount Employer" + Abs(Deduct."Share Top Up");
                                        end;
                                end;
                            end;
                        end;
    ...
    var
        ...
        BenevolentAmt: Decimal;
        BenevolentBal: Decimal;
