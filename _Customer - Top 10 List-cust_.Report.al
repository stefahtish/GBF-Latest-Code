report 50148 "Customer - Top 10 List-cust"
{
    DefaultLayout = RDLC;
    RDLCLayout = './CustomerTop10List - Cust.rdlc';
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            begin
                ...
                if ("Sales (LCY)" = 0) and ("Balance (LCY)" = 0) then CurrReport.Skip();
                CustAmount.Init();
        ...
        dataitem("Integer"; "Integer")
        {
            ...
            column(SalesLCYCaption; SalesLCYCaption)
            {
            }
            column(BalanceLCYCaption; BalanceLCYCaption)
            {
            }
            ...
            begin
                if Number = 1 then begin
                    if not CustAmount.Find('-') then CurrReport.Break();
                end
                else if CustAmount.Next = 0 then CurrReport.Break();
                CustAmount."Amount (LCY)" := -CustAmount."Amount (LCY)";
                Customer.Get(CustAmount."Customer No.");
                Customer.CalcFields("Sales (LCY)", "Balance (LCY)");
                if MaxAmount = 0 then MaxAmount := CustAmount."Amount (LCY)";
                CustAmount."Amount (LCY)" := -CustAmount."Amount (LCY)";
            end;
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(Show; ShowType)
                    {
                        ...
                        Caption = 'Show';
                        OptionCaption = 'Sales (KES),Balance (KES)';
                    ...
                    field(NoOfRecordsToPrint; NoOfRecordsToPrint)
                    {
                        ...
                        trigger OnValidate()
                        begin
                            if NoOfRecordsToPrint <= 0 then Error(NoOfRecordsToPrintErrMsg);
                        end;
                    }
    ...
    var
        ...
        i: Integer;
        TotalSales: Decimal;
        Text004: Label 'Sales (KES),Balance (KES)';
        ...
        SalesLCYCaption: Label 'Sales (KES)';
        BalanceLCYCaption: Label 'Balance (KES)';
