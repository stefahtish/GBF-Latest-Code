report 50143 "Customer Detailed Aging-cust"
{
    DefaultLayout = RDLC;
    RDLCLayout = './CustomerDetailedAging-cust.rdlc';
    ...
    dataset
    {
        dataitem(Header; "Integer")
        {
            ...
            column(Cust_Ledger_Entry_Remaining_Amt_LCY_Caption; RemainingAmntCaption)
            {
            }
            ...
            dataitem(Customer; Customer)
            {
                ...
                dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
                {
                    ...
                    begin
                        ...
                        TempCurrencyTotalBuffer.UpdateTotal("Currency Code", "Remaining Amount", "Remaining Amt. (LCY)", Counter);
                    end;
                ...
                dataitem("Integer"; "Integer")
                {
                    ...
                    begin
                        ...
                        TempCurrencyTotalBuffer2.UpdateTotal(TempCurrencyTotalBuffer."Currency Code", TempCurrencyTotalBuffer."Total Amount", TempCurrencyTotalBuffer."Total Amount (LCY)", Counter1);
                    end;
                ...
                trigger OnAfterGetRecord()
                begin
                    if not CustomersWithLedgerEntriesList.Contains("No.") then CurrReport.Skip();
                end;

                trigger OnPreDataItem()
                begin
                    if OnlyOpen then NumCustLedgEntriesperCust.SetFilter(OpenValue, 'TRUE');
                    if NumCustLedgEntriesperCust.Open then while NumCustLedgEntriesperCust.Read do if not CustomersWithLedgerEntriesList.Contains(NumCustLedgEntriesperCust.Customer_No) then CustomersWithLedgerEntriesList.Add(NumCustLedgEntriesperCust.Customer_No);
                end;
            }
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field("Start Date"; Startdate)
                    {
                        ...
                        Caption = 'Start Date';
                        ToolTip = 'Specifies the Start of the period covered by the report (for example, 12/31/17).';
                    }
    ...
    begin
        ...
        if EndDate = 0D then Error(ErrorEmptyStartdate);
    ...
    var
        ...
        RemainingAmntCaption: Label 'Remaining Amt. (KES)';
        Startdate: date;
        ErrorEmptyStartdate: label 'Start date must have a value';
        ErrorEmptyEnddate: label 'Ending date must have a value';
    ...
    begin
        ...
        if Date2DMY(FromDate, 1) - LeftOverDays <= 1 then FullMonths += 1;
        exit(FullMonths);
    end;
}
