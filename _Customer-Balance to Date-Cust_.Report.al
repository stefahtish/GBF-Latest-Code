report 50145 "Customer-Balance to Date-Cust"
{
    DefaultLayout = RDLC;
    RDLCLayout = './CustomerBalancetoDate-cust.rdlc';
        ...
        dataitem(Customer; Customer)
        {
            ...
            dataitem(CustLedgEntry3; "Cust. Ledger Entry")
            {
                ...
                begin
                    ...
                    if FindSet then
                        repeat
                            if not Open then ClosedEntryIncluded := CheckCustEntryIncluded("Entry No.");
            ...
            dataitem(Integer2; "Integer")
            {
                ...
                begin
                    ...
                    CurrencyTotalBuffer2.UpdateTotal(CurrencyTotalBuffer."Currency Code", CurrencyTotalBuffer."Total Amount", 0, Counter1);
                end;
                ...
                trigger OnPreDataItem()
                begin
                    if not ShowEntriesWithZeroBalance then CurrencyTotalBuffer.SetFilter("Total Amount", '<>0');
                end;
            }
            trigger OnAfterGetRecord()
            begin
                if MaxDate = 0D then Error(BlankMaxDateErr);
                SetRange("Date Filter", 0D, MaxDate);
                CalcFields("Net Change (LCY)", "Net Change");
                if ("Net Change (LCY)" = 0) and ("Net Change" = 0) and (not ShowEntriesWithZeroBalance) then CurrReport.Skip();
            end;
        }
    ...
    var
        ...
        AllamtsareinLCYCaptionLbl: Label 'All amounts are in KES.';
    ...
    begin
        if Customer.GetFilter("Global Dimension 1 Filter") <> '' then CustLedgerEntry.SetFilter("Global Dimension 1 Code", Customer.GetFilter("Global Dimension 1 Filter"));
        if Customer.GetFilter("Global Dimension 2 Filter") <> '' then CustLedgerEntry.SetFilter("Global Dimension 2 Code", Customer.GetFilter("Global Dimension 2 Filter"));
        if Customer.GetFilter("Currency Filter") <> '' then CustLedgerEntry.SetFilter("Currency Code", Customer.GetFilter("Currency Filter"));
    end;
    ...
    begin
        ...
        if TempCustLedgerEntry.FindSet then
            repeat
                ...
                if RemainingAmt <> 0 then CurrencyTotalBuffer.UpdateTotal(CurrencyCode, RemainingAmt, 0, Counter1);
    ...
    begin
        if CustLedgerEntry.Get(EntryNo) and (CustLedgerEntry."Posting Date" <= MaxDate) then begin
            ...
            if CustLedgerEntry."Remaining Amount" <> 0 then exit(true);
            if PrintUnappliedEntries then exit(CheckUnappliedEntryExists(EntryNo));
        end;
        exit(false);
    end;
