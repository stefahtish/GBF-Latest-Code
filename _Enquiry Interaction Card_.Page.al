page 50721 "Enquiry Interaction Card"
{
                ...
                action(Acknowledge)
                {
                    ...
                    begin
                        if confirm('Are you sure?', false) then begin
                            ...
                            SenderName := CompanyName;
                            Rec.CalcFields("Problem Reported");
                            AcknowledgeBody := 'The enquiry ' + ProblemNotesText + ' has been received and is being acted upon';
                            Subject := Format(Rec."Interaction Type");
                            SMTP.Create(Receipients, Subject, '', true);
                            SMTP.AppendtoBody(AcknowledgeBody);
                            email.Send(SMTP);
                            //Notify MD
                            AcknowledgeBody := 'The enquiry ' + ProblemNotesText + 'by' + Rec."Client Name" + 'has been received and is being acted upon';
                            Clear(Receipients);
    ...
    var
        myInt: Integer;
    begin
        Rec."Interaction Type" := Rec."Interaction Type"::Enquiry;
    end;
