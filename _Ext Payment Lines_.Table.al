table 50182 "Ext Payment Lines"
{
    fields
    {
        ...
        field(8; Amount; Decimal)
        {
            ...
            trigger OnValidate()
            var
                PmtLines: Record "Ext Payment Lines";
            ...
            begin
                ...
                if Pmts2.FindFirst()then begin
                    ...
                    if "Line Type" = "Line Type"::Normal then begin
                        ...
                        if Pmts.Find('-')then repeat PmtLines.Reset();
                                ...
                                if PmtLines.Find('-')then repeat UsedAmt+=PmtLines.Amount;
                                    until PmtLines.Next() = 0;
                            until Pmts.next = 0;
                    end;
                    if "Line Type" = "Line Type"::Item then begin
                        repeat PmtLines.Reset();
                    ...
                    if "VAT Code" <> '' then begin
                        ...
                        if VATSetup.Get(GLAccount."VAT Bus. Posting Group", "VAT Code")then begin
                            if VATSetup."VAT %" <> 0 then begin
                                if "VAT Exclusive" = false then VATAmount:=Round(((Amount) / (1 + VATSetup."VAT %" / 100) * VATSetup."VAT %" / 100), CSetup."Rounding Precision", Direction)
                                else
                ...
                "Account Type"::Vendor: begin
                    if "VAT Code" <> '' then begin
                        if Vendor.Get("Account No")then if VATSetup.Get(Vendor."VAT Bus. Posting Group", "VAT Code")then begin
                                if VATSetup."VAT %" <> 0 then begin
                                    if "Vatable Amount" = 0 then VATAmount:=Round(((Amount) / (1 + VATSetup."VAT %" / 100) * VATSetup."VAT %" / 100), CSetup."Rounding Precision", Direction)
                                    else
 ...
 "Net Amount":=NetAmount
                                    ...
                                    if "W/T VAT Code" <> '' then begin //Compute W/T VAT
                                        if Vendor.Get("Account No")then if VATSetup.Get(Vendor."VAT Bus. Posting Group", "W/T VAT Code")then begin
                ...
                if Currency = '' then "Amount (LCY)":=Round(Amount, CurrencyRec."Amount Rounding Precision")
                else
                begin
        ...
        field(95; "Expenditure Type"; Code[20])
        {
            ...
            begin
                ...
                case PaymentRec."Payment Type" of PaymentRec."Payment Type"::"Payment Voucher": RecTypes.SetRange(Type, RecTypes.Type::Payment);
                PaymentRec."Payment Type"::"Petty Cash": RecTypes.SetRange(Type, RecTypes.Type::"Petty Cash");
                PaymentRec."Payment Type"::Imprest: RecTypes.SetRange(Type, RecTypes.Type::Imprest);
                PaymentRec."Payment Type"::"Staff Claim": RecTypes.SetRange(Type, RecTypes.Type::Claim);
                PaymentRec."Payment Type"::Receipt: RecTypes.SetRange(Type, RecTypes.Type::Receipt);
                PaymentRec."Payment Type"::"Input Tax": RecTypes.SetRange(Type, RecTypes.Type::"Input Tax");
                PaymentRec."Payment Type"::"Service Charge", PaymentRec."Payment Type"::"Service Charge Claim": RecTypes.SetRange(Type, RecTypes.Type::"Service Charge");
                end;
        ...
        field(97; Name; Text[250])
        {
            DataClassification = ToBeClassified;
        }
        field(98; "Salary Scale"; Code[30])
        {
            TableRelation = "Salary Scale".Scale;

            trigger OnValidate()
            begin
                //  Amount := GetDestinationRate2("Salary Scale");
                CurrencyRec.InitRoundingPrecision;
                if Currency = '' then "Amount (LCY)":=Round(Amount, CurrencyRec."Amount Rounding Precision")
                else
                begin
                        ...
                        if WorkProgramme."Created By" = UserId then begin
                            ...
                            if Items.Find('-')then begin
                                repeat UsedAmt:=0;
                                    ...
                                    if PmtLines.Find('-')then repeat Qty+=PmtLines.Quantity;
                                        //Message('%1', Qty);
                                        until PmtLines.Next() = 0;
    ...
    begin
        ...
        if FieldNumber = 5 then begin
            ...
            if GLBudget.Find('-')then begin
                //Global Dimensions
                OldDimSetID:="Dimension Set ID";
                if PaymentRec."Multi-Donor" then DimMgt.ValidateShortcutDimValues(1, "Shortcut Dimension 1 Code", "Dimension Set ID")
                else
                ...
                OldDimSetID:="Dimension Set ID";
                if PaymentRec."Multi-Donor" then DimMgt.ValidateShortcutDimValues(2, "Shortcut Dimension 2 Code", "Dimension Set ID")
                else
    ...
    procedure CheckDocType(): Boolean begin
        PaymentRec.Reset;
        PaymentRec.SetRange("Payment Type", "Payment Type");
        PaymentRec.SetRange("No.", No);
        if PaymentRec.Find('-')then begin
            if PaymentRec."Multi-Donor" then exit(true)
            else
                exit(false);
        end;
    end;
    procedure ImprestLineExist(): Boolean var
        ImprestRec: Record Payments;
        ImprestLines: Record "Ext Payment Lines";
    begin
        ImprestRec.Reset;
    ...
    begin
        ...
        PaymentRec.SetRange(PaymentRec."No.", No);
        if PaymentRec.FindFirst then begin
            case PaymentRec."Payment Type" of PaymentRec."Payment Type"::"Bank Transfer": "Payment Type":="Payment Type"::"Bank Transfer";
            PaymentRec."Payment Type"::Imprest: begin
                "Payment Type":="Payment Type"::Imprest;
                "Account Type":="Account Type"::"G/L Account";
            //"No of Days":=GetNoOfDays;
            end;
            PaymentRec."Payment Type"::"Imprest Surrender": begin
                "Payment Type":="Payment Type"::"Imprest Surrender";
                "Account Type":="Account Type"::"G/L Account";
            end;
            PaymentRec."Payment Type"::"Payment Voucher": "Payment Type":="Payment Type"::"Payment Voucher";
            PaymentRec."Payment Type"::"Petty Cash": begin
                "Payment Type":="Payment Type"::"Petty Cash";
                "Account Type":="Account Type"::"G/L Account";
            end;
            PaymentRec."Payment Type"::"Petty Cash Surrender": begin
                "Payment Type":="Payment Type"::"Petty Cash Surrender";
                "Account Type":="Account Type"::"G/L Account";
            end;
            PaymentRec."Payment Type"::Receipt: "Payment Type":="Payment Type"::Receipt;
            PaymentRec."Payment Type"::"Staff Advance": begin
                "Payment Type":="Payment Type"::"Staff Advance";
                "Account Type":="Account Type"::"G/L Account";
            end;
            PaymentRec."Payment Type"::"Receipt-Property": "Payment Type":="Payment Type"::"Receipt-Property";
            PaymentRec."Payment Type"::"Staff Claim": begin
                "Payment Type":="Payment Type"::"Staff Claim";
                "Account Type":="Account Type"::"G/L Account";
            end;
            PaymentRec."Payment Type"::"Input Tax": "Payment Type":="Payment Type"::"Input Tax";
            PaymentRec."Payment Type"::"Service Charge": "Payment Type":="Payment Type"::"Service Charge";
            PaymentRec."Payment Type"::"Service Charge Surrender": "Payment Type":="Payment Type"::"Service Charge Surrender";
            PaymentRec."Payment Type"::"Service Charge Claim": "Payment Type":="Payment Type"::"Service Charge Claim";
            end;
    ...
    begin
        ...
        if Pmts."Claim Type" <> Pmts."Claim Type"::"Mileage Claim" then begin
            ...
            if DestinationSetup.FindFirst then begin
                ...
                DestinationRate.SetRange("Employee Job Group", "Salary Scale");
    ...
    local procedure GetDestinationRate2(SalaryScale: Code[30]): Decimal var
        TotalAmount: Decimal;
        DestinationSetup: Record Destination;
        DestinationRate: Record "Destination Rate Entry";
        Pmts: Record Payments;
    begin
        ...
        if Pmts."Claim Type" <> Pmts."Claim Type"::"Mileage Claim" then begin
            ...
            if DestinationSetup.FindFirst then begin
                DestinationRate.Reset;
                DestinationRate.SetRange("Destination Code", Pmts.Destination);
                DestinationRate.SetRange("Employee Job Group", SalaryScale);
                DestinationRate.SetRange("Advance Code", "Expenditure Type");
                if DestinationRate.FindFirst then begin
                    "Daily Rate":=DestinationRate."Daily Rate (Amount)";
                    TotalAmount:="Daily Rate" * "No of Days";
    ...
    local procedure GetNoOfDays(): Integer begin
        if PaymentRec.Get(No)then exit(PaymentRec."No of Days");
    end;
    procedure GetAppliedDoc(): Code[50]var
