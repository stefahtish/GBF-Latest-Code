page 50229 "FA Marked For Disposal Card"
{
    PageType = Document;
            ...
            group(General)
            {
                ...
                group(Control34)
                {
                    ...
                    field("FA Subclass Code"; Rec."FA Subclass Code")
                    {
                        ...
                        begin
                            if Rec."FA Class Code" <> '' then FASubclass.SetFilter("FA Class Code", '%1|%2', '', Rec."FA Class Code");
    ...
    begin
        if Rec."No." <> xRec."No." then SaveSimpleDepriciationBook(xRec."No.");
        LoadDepreciationBooks;
    ...
    begin
        ShowAcquireNotification := (not Rec.Acquired) and Rec.FieldsForAcquitionInGeneralGroupAreCompleted and AtLeastOneDepreciationLineIsComplete;
    ...
    begin
        if Simple then exit(FADepreciationBook.RecIsReadyForAcquisition);
    ...
    begin
        if not SimpleDepreciationBookHasChanged then exit;
    ...
    var
        FASubclass: Record "FA Subclass";
    begin
        if FADepreciationBook."FA Posting Group" <> '' then exit;
        if FASubclass.Get(Rec."FA Subclass Code") then;
