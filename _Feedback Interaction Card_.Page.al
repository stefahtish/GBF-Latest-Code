page 50722 "Feedback Interaction Card"
{
                ...
                action(Acknowledge)
                {
                    ...
                    begin
                        if confirm('Are you sure?', false) then begin
                            ...
                            SenderName := CompanyName;
                            Rec.CalcFields("Problem Reported");
                            AcknowledgeBody := 'The feedback ' + ProblemNotesText + ' has been received and is being acted upon';
                            Subject := Format(Rec."Interaction Type");
                            SMTP.Create(Receipients, Subject, '', true);
                            SMTP.AppendtoBody(AcknowledgeBody);
                            Email.send(smtp);
                            //Notify MD
                            AcknowledgeBody := 'The feedback ' + ProblemNotesText + 'by' + Rec."Client Name" + 'has been received and is being acted upon';
                            Clear(Receipients);
