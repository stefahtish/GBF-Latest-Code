report 50166 "Immprest Warranty"
{
    DefaultLayout = Word;
    RDLCLayout = './ImprestWarranty.rdl';
    WordLayout = './ImprestWarranty.docx';
    ApplicationArea = All;
    ...
    dataset
    {
        dataitem(Payments; Payments)
        {
            ...
            column(FinalAuthorizer; GetUserName(Approver[5]))
            {
            }
            column(FinalDateAuthorized; ApproverDate[5])
            {
            }
            column(FinalAuthorizer_Signature; UserSetup4.Signature)
            {
            }
            ...
            begin
                ...
                //Approvals
                Imprest.Reset();
                Imprest.SetRange("No.", "Imprest Issue Doc. No");
                ...
                if ApprovalEntries.Find('-') then begin
                    repeat
                        ...
                        if ApprovalEntries."Sequence No." = 4 then begin
                            ...
                            if UserSetup4.Get(Approver[5]) then UserSetup4.CalcFields(Signature);
                        end;
