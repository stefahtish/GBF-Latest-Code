table 50127 "Internal Request Line"
{
    ...
    fields
    {
        ...
        field(11; Description; Text[250])
        {
            ...
            begin
                ...
                case Type of
                    Type::Item:
                        if "No." = '' then begin
                            if Item.TryGetItemNo(ReturnValue, Description, false) then
                                if ReturnValue = "No." then
        ...
        field(23; "Unit Cost (LCY)"; Decimal)
        {
            ...
            begin
                ...
                if ("Direct Unit Cost" <> 0) and ("Direct Unit Cost" <> ("Line Discount Amount" / Quantity)) then
                    "Indirect Cost %" := Round((UnitCostCurrency - "Direct Unit Cost" + "Line Discount Amount" / Quantity) / ("Direct Unit Cost" - "Line Discount Amount" / Quantity) * 100, 0.00001)
                else
                    "Indirect Cost %" := 0;
                UpdateSalesCost;
                if JobTaskIsSet then begin
            ...
            begin
                ...
                if xRec."Line Discount Amount" <> "Line Discount Amount" then
                    if Round(Quantity * "Direct Unit Cost", Currency."Amount Rounding Precision") <> 0 then
                        "Line Discount %" := Round("Line Discount Amount" / Round(Quantity * "Direct Unit Cost", Currency."Amount Rounding Precision") * 100, 0.00001)
                    else
                        "Line Discount %" := 0;
                "Inv. Discount Amount" := 0;
                "Inv. Disc. Amount to Invoice" := 0;
            ...
            begin
                ...
                case "VAT Calculation Type" of
                    ...
                    "VAT Calculation Type"::"Full VAT":
                        if Amount <> 0 then
                            FieldError(Amount, StrSubstNo(Text011, FieldCaption("VAT Calculation Type"), "VAT Calculation Type"));
        ...
        field(57; "Outstanding Amount"; Decimal)
        {
            ...
            begin
                ...
                if PurchHeader."Currency Code" <> '' then
                    "Outstanding Amount (LCY)" := Round(CurrExchRate.ExchangeAmtFCYToLCY(GetDate, "Currency Code", "Outstanding Amount", PurchHeader."Currency Factor"), Currency2."Amount Rounding Precision")
                else
        ...
        field(59; "Amt. Rcd. Not Invoiced"; Decimal)
        {
            ...
            begin
                ...
                Currency2.InitRoundingPrecision;
                if PurchHeader."Currency Code" <> '' then
                    "Amt. Rcd. Not Invoiced (LCY)" := Round(CurrExchRate.ExchangeAmtFCYToLCY(GetDate, "Currency Code", "Amt. Rcd. Not Invoiced", PurchHeader."Currency Factor"), Currency2."Amount Rounding Precision")
                else
        ...
        field(5793; "Inbound Whse. Handling Time"; DateFormula)
        {
            ...
            begin
                TestStatusOpen;
                if ("Promised Receipt Date" <> 0D) or ("Requested Receipt Date" <> 0D) then
                    Validate("Planned Receipt Date")
                else
        ...
        field(5807; "Return Shpd. Not Invd."; Decimal)
        {
            ...
            begin
                ...
                Currency2.InitRoundingPrecision;
                if PurchHeader."Currency Code" <> '' then
                    "Return Shpd. Not Invd. (LCY)" := Round(CurrExchRate.ExchangeAmtFCYToLCY(GetDate, "Currency Code", "Return Shpd. Not Invd.", PurchHeader."Currency Factor"), Currency2."Amount Rounding Precision")
                else
        ...
        field(54022; "Procurement Plan Item"; Code[20])
        {
            ...
            begin
                if ProcurementPlan.Get("Procurement Plan", "Shortcut Dimension 1 Code", "Shortcut Dimension 2 Code", "Procurement Plan Item") then begin
                    case ProcurementPlan."Procurement Type" of
                        ProcurementPlan."Procurement Type"::Goods:
                            begin
                                Type := Type::Item;
                                "No." := ProcurementPlan."No.";
                            end;
                        ProcurementPlan."Procurement Type"::Service, ProcurementPlan."Procurement Type"::Works:
                            begin
                                Type := Type::"G/L Account";
                                "No." := ProcurementPlan."Source of Funds";
                            end;
                        ProcurementPlan."Procurement Type"::"Fixed Asset":
                            begin
        ...
        field(54048; "Vendor Type"; Option)
        {
            DataClassification = ToBeClassified;
            OptionMembers = " ","Existing","External";
        }
        field(54049; "ExternalSupplier Name"; code[200])
        {
            DataClassification = ToBeClassified;

        }
        field(54059; "Date Received"; date)
        {
            DataClassification = ToBeClassified;

        }
        field(54050; "Quote Amount"; Decimal)
        {
            DataClassification = ToBeClassified;
    ...
    begin
        if Quantity = 0 then begin
            ...
            if "Document Type" in ["Document Type"::"Return Order"] then
                Validate("Return Shpd. Not Invd.", Round(AmountInclVAT * "Return Qty. Shipped Not Invd." / Quantity, Currency."Amount Rounding Precision"))
            else
    ...
    begin
        ...
        if "VAT Calculation Type" = "VAT Calculation Type"::"Full VAT" then
            ...
            "Unit Cost" := ("Direct Unit Cost" - DiscountAmountPerQty) * (1 + "Indirect Cost %" / 100) / (1 + "VAT %" / 100) + GetOverheadRateFCY - "VAT Difference"
        else
    ...
    begin
        ...
        if "Line Amount" = "Inv. Discount Amount" then begin
            ...
            TotalQuantityBase := 0;
            if ("VAT Calculation Type" = "VAT Calculation Type"::"Sales Tax") or (("VAT Calculation Type" in ["VAT Calculation Type"::"Normal VAT", "VAT Calculation Type"::"Reverse Charge VAT"]) and ("VAT %" <> 0)) then
    ...
    begin
        ...
        GetPurchHeader;
        if PurchHeader."Currency Code" <> '' then
            "Unit Cost" := CurrExchRate.ExchangeAmtLCYToFCY(GetDate, PurchHeader."Currency Code", "Unit Cost (LCY)", PurchHeader."Currency Factor")
        else
    ...
    begin
        case "Document Type" of
            "Document Type"::Stock, "Document Type"::Purchase, "Document Type"::CAPEX, "Document Type"::RFQ:
                exit(Value);
    ...
    begin
        ...
        if PurchHeader."Currency Code" = '' then
            ...
            Currency.Get(PurchHeader."Currency Code");
        if ("Inv. Discount Amount" = 0) and ("Line Discount Amount" = 0) and (not PurchHeader."Prices Including VAT") then
    ...
    begin
        ...
        else if "Requested Receipt Date" <> 0D then
            Validate("Requested Receipt Date")
        else
            GetUpdateBasicDates;
    end;
    ...
    begin
                ...
                if not PurchLine.ZeroAmountLine(QtyType) then begin
                    ...
                    case QtyType of
                        ...
                        QtyType::Invoicing:
                            begin
                                case true of
                                    (PurchLine."Document Type" in [PurchLine."Document Type"::Order, PurchLine."Document Type"::Invoice]) and (not PurchHeader.Receive) and PurchHeader.Invoice and (not PurchLine."Prepayment Line"):
                                    ...
                                    (PurchLine."Document Type" in [PurchLine."Document Type"::"Return Order", PurchLine."Document Type"::"Credit Memo"]) and (not PurchHeader.Ship) and PurchHeader.Invoice:
        ...
        if RoundingLineInserted and (TotalVATAmount <> 0) then
            if VATAmountLine.Get(PurchLine."VAT Identifier", PurchLine."VAT Calculation Type", PurchLine."Tax Group Code", PurchLine."Use Tax", PurchLine."Line Amount" >= 0) then begin
    ...
    begin
        if Type = Type::Item then
            case true of
                ("Document Type" in ["Document Type"::Stock, "Document Type"::Purchase]) and (Quantity >= 0):
    ...
    begin
        ...
        if ("Document Type" in ["Document Type"::Purchase]) and Location2."Directed Put-away and Pick" then begin
            ...
            if (("Document Type" = "Document Type"::Purchase) and (Quantity >= 0)) or (Quantity < 0) then
            ...
            if (("Document Type" = "Document Type"::Purchase) and (Quantity >= 0) and (Location2."Require Receive" or Location2."Require Put-away")) or ((Quantity < 0) and (Location2."Require Receive" or Location2."Require Put-away")) then begin
                if WhseValidateSourceLine.WhseLinesExist(DATABASE::"Purchase Line", "Document Type", "Document No.", "Line No.", 0, Quantity) then
                    ShowDialog := ShowDialog::Error
    ...
    begin
        case "Document Type" of
            "Document Type"::Purchase, "Document Type"::CAPEX, "Document Type"::Stock, "Document Type"::RFQ:
    ...
    begin
        if ("Qty. to Issue" <> 0) and ("Prepmt. Amt. Inv." <> 0) then begin
            ...
            if ("Prepayment %" = 100) and not IsFinalInvoice then
                "Prepmt Amt to Deduct" := GetLineAmountToHandle("Qty. to Issue")
            else
                "Prepmt Amt to Deduct" := Round(("Prepmt. Amt. Inv." - "Prepmt Amt Deducted") * "Qty. to Issue" / (Quantity - "Quantity Issued"), Currency."Amount Rounding Precision")
        end
        else
    ...
    begin
        if "Document No." <> '' then begin
            ...
            if (PurchHeader.Status = PurchHeader.Status::Released) and (xRec.Type in [xRec.Type::Item, xRec.Type::"Fixed Asset"]) then
                Type := Type::" "
            else
                Type := xRec.Type;
        end;
                ...
                Type::Item:
                    begin
                        ...
                        case ICPartner."Outbound Purch. Item No. Type" of
                            ...
                            ICPartner."Outbound Purch. Item No. Type"::"Internal No.", ICPartner."Outbound Purch. Item No. Type"::"Cross Reference":
                                begin
    ...
    begin
        if "Document Type" in ["Document Type"::Purchase, "Document Type"::CAPEX, "Document Type"::"Return Order"] then
            case Type of
                Type::"G/L Account":
