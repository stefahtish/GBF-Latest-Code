report 50259 "My Payslip"
{
    // ArrEarnings[1,1]
    // ArrEarningsAmt[1,1]
    DefaultLayout = RDLC;
    RDLCLayout = './MyPayslip.rdlc';
    ApplicationArea = All;
    ...
    dataset
    {
        dataitem(Employee; Employee)
        {
            ...
            begin
                if Employee."No." <> StaffNo then Error('Kindly Select your staff Number');
                //Pickup Logo
                if Employee.Company <> '' then CompInfo.ChangeCompany(Employee.Company);
                CompInfo.Get;
                    ...
                    repeat
                        ...
                        if LoanBalances.Find('-') then begin
                            case LoanBalances."Interest Calculation Method" of
                                LoanBalances."Interest Calculation Method"::"Reducing Balance", LoanBalances."Interest Calculation Method"::"Flat Rate":
                                    begin
                        ...
                        if Deduct.Get(AssignMatrix.Code) then begin
                            if Deduct."Show Balance" then begin
                                ...
                                IF DeductionBalances.FIND('-') THEN BEGIN
                                    DeductionBalances.CALCSUMS(Amount);
                                    IF Deduct."Show Balance" THEN BEGIN
                                        CASE Deduct."Balance Type" OF
                                            Deduct."Balance Type"::Increasing:
                                                BEGIN
                                                    //Added to filter balance by date specified in opening balance
                                                    DeductCopy.COPY(Deduct);
                                                    DeductCopy.SETRANGE("Pay Period Filter", DeductionBalances.Date, DateSpecified);
                                                    DeductCopy.CALCFIELDS(DeductCopy."Total Amount", DeductCopy."Total Amount Employer", DeductCopy."Share Top Up");
                                                    IF Deduct."Exclude Employer Balance" THEN
                                                        BalanceArray[1, i] := (DeductionBalances.Amount + ABS(DeductCopy."Total Amount") + ABS(DeductCopy."Share Top Up"))
                                                    ELSE
                                                        BalanceArray[1, i] := (DeductionBalances.Amount + ABS(DeductCopy."Total Amount") + ABS(DeductCopy."Total Amount Employer") + ABS(DeductCopy."Share Top Up"));
                                                END;
                                            Deduct."Balance Type"::Decreasing:
                                                BEGIN
                                                    ...
                                                    DeductCopy.SETRANGE("Pay Period Filter", DeductionBalances.Date, DateSpecified);
                                                    DeductCopy.CALCFIELDS(DeductCopy."Total Amount", DeductCopy."Total Amount Employer", DeductCopy."Share Top Up");
                                                    IF DeductCopy."Exclude Employer Balance" THEN
                                                        BalanceArray[1, i] := DeductionBalances.Amount + DeductCopy."Total Amount" + ABS(DeductCopy."Share Top Up")
                                                    ELSE
                                                        BalanceArray[1, i] := DeductionBalances.Amount + DeductCopy."Total Amount" + DeductCopy."Total Amount Employer" + ABS(DeductCopy."Share Top Up");
                                                END;
                                        END;
                                    END;
                                END;
    ...
    begin
        MyUserID := UserId;
        if UserSetup.Get(MyUserID) then StaffNo := UserSetup."Employee No.";
