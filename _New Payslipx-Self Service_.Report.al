report 50462 "New Payslipx-Self Service"
{
    // ArrEarnings[1,1]
    // ArrEarningsAmt[1,1]
    DefaultLayout = RDLC;
    RDLCLayout = './NewPayslipxSelfService.rdl';
    ApplicationArea = All;
    ...
    dataset
    {
        dataitem(Employee; Employee)
        {
            ...
            begin
                ...
                Addr[1] [7] := Format(Employee."ID No.");
                // get Department Name
                DimVal.Reset;
        ...
        layout
        {
            area(Content)
            {
                field(RequestPayrollPeriod; RequestPayrollPeriod)
                {
                    ...
                    TableRelation = "Payroll PeriodX"."Starting Date" where(Closed = filter(true));
                    ApplicationArea = All;
                }
            }
        }
    ...
    begin
        ...
        if PayPeriod.FindLast() then RequestPayrollPeriod := PayPeriod."Starting Date";
    end;
