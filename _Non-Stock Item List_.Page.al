page 51341 "Non-Stock Item List"
{
    ...
    Caption = 'Non-Stock Item List';
    CardPageID = "Non-Stock Items";
    Editable = false;
    PageType = List;
    ...
    SourceTable = Item;
    UsageCategory = Lists;
    SourceTableView = where(Type=const("Non-Inventory"));
        ...
        area(processing)
        {
            group(Item)
            {
                ...
                action(FilterByAttributes)
                {
                    ...
                    begin
                        ...
                        if ClientTypeManagement.GetCurrentClientType = CLIENTTYPE::Phone then FilterPageID:=PAGE::"Filter Items by Att. Phone";
                        CloseAction:=PAGE.RunModal(FilterPageID, TempFilterItemAttributesBuffer);
                        if(ClientTypeManagement.GetCurrentClientType <> CLIENTTYPE::Phone) and (CloseAction <> ACTION::LookupOK)then exit;
            ...
            group(History)
            {
                ...
                group("E&ntries")
                {
                    ...
                    action("Ledger E&ntries")
                    {
                        ...
                        RunPageLink = "Item No."=FIELD("No.");
                        RunPageView = SORTING("Item No.")ORDER(Descending);
                        Scope = Repeater;
                        ShortCutKey = 'Ctrl+F7';
            ...
            group(RequestApproval)
            {
                ...
                action(SendApprovalRequest)
                {
                    ...
                    begin
                        if ApprovalsMgmt.CheckItemApprovalsWorkflowEnabled(Rec)then ApprovalsMgmt.OnSendItemForApproval(Rec);
                    end;
                }
            ...
            group(Display)
            {
                ...
                action(ReportFactBoxVisibility)
                {
                    ...
                    //eddie CurrPage."Power BI Report FactBox".PAGE.SetFactBoxVisibility(PowerBIVisible);
                    end;
                }
            }
        }
            ...
            group(ActionGroupCRM)
            {
                ...
                action(CRMSynchronizeNow)
                {
                    ...
                    begin
                        CurrPage.SetSelectionFilter(Item);
                        Item.Next;
                        if Item.Count = 1 then CRMIntegrationManagement.UpdateOneNow(Item.RecordId)
                        else
                        begin
    ...
    var
        ...
        WorkflowWebhookManagement: Codeunit "Workflow Webhook Management";
    begin
        if SocialListeningSetupVisible then //SocialListeningMgt.GetItemFactboxVisibility(Rec, SocialListeningSetupVisible, SocialListeningVisible);
            if CRMIntegrationEnabled then CRMIsCoupledToRecord:=CRMCouplingManagement.IsRecordCoupledToCRM(Rec.RecordId);
    ...
    //eddie  CurrPage."Power BI Report FactBox".PAGE.SetCurrentListSelection(Rec."No.", false, PowerBIVisible);
    end;
    ...
    begin
        if RunOnTempRec then begin
            ...
            if Found then Rec:=TempItemFilteredFromAttributes;
            exit(Found);
        end;
        exit(Rec.Find(Which));
    end;
    trigger OnInit()
    begin
    //eddie   CurrPage."Power BI Report FactBox".PAGE.InitFactBox(CurrPage.ObjectId(false), CurrPage.Caption, PowerBIVisible);
    end;
    trigger OnNextRecord(Steps: Integer): Integer var
        ResultSteps: Integer;
    begin
        if RunOnTempRec then begin
            TempItemFilteredFromAttributes.Copy(Rec);
            ResultSteps:=TempItemFilteredFromAttributes.Next(Steps);
            if ResultSteps <> 0 then Rec:=TempItemFilteredFromAttributes;
            exit(ResultSteps);
        end;
        exit(Rec.Next(Steps));
    end;
    ...
    begin
        ...
        if CRMIntegrationEnabled then if IntegrationTableMapping.Get('ITEM-PRODUCT')then BlockedFilterApplied:=IntegrationTableMapping.GetTableFilter().Contains('Field54=1(0)');
    ...
    //eddie  ItemReferenceVisible := ItemReferenceMgt.IsEnabled();
    end;
    var TempFilterItemAttributesBuffer: Record "Filter Item Attributes Buffer" temporary;
    ...
    protected var IsFoundationEnabled: Boolean;
    ...
    procedure SelectActiveItems(): Text var
        Item: Record Item;
    begin
        exit(SelectInItemList(Item));
    end;
    procedure SelectActiveItemsForSale(): Text var
        Item: Record Item;
    begin
        Item.SetRange("Sales Blocked", false);
        exit(SelectInItemList(Item));
    end;
    procedure SelectActiveItemsForPurchase(): Text var
        Item: Record Item;
    begin
        Item.SetRange("Purchasing Blocked", false);
        exit(SelectInItemList(Item));
    end;
    procedure SelectInItemList(var Item: Record Item): Text var
        ItemListPage: Page "Item List";
    begin
        Item.SetRange(Blocked, false);
        ItemListPage.SetTableView(Item);
        ItemListPage.LookupMode(true);
        if ItemListPage.RunModal = ACTION::LookupOK then exit(ItemListPage.GetSelectionFilter);
    end;
    ...
    begin
        EventFilter:=WorkflowEventHandling.RunWorkflowOnSendItemForApprovalCode + '|' + WorkflowEventHandling.RunWorkflowOnItemChangedCode;
    ...
    begin
        ...
        RunOnTempRec:=true;
        RunOnPickItem:=true;
        if Item.FindSet then repeat TempItemFilteredFromAttributes:=Item;
    ...
    begin
        if not RunOnPickItem then exit;
        TempItemFilteredFromAttributes.Reset();
        TempItemFilteredFromAttributes.DeleteAll();
        RunOnTempRec:=true;
        if TempItemFilteredFromPickItem.FindSet then repeat TempItemFilteredFromAttributes:=TempItemFilteredFromPickItem;
                TempItemFilteredFromAttributes.Insert();
