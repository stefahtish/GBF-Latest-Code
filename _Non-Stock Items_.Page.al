page 51340 "Non-Stock Items"
{
    Caption = 'Non-Stock Item Card';
    PageType = Card;
    SourceTable = Item;
    ApplicationArea = All;
        ...
        area(content)
        {
            ...
            group(InventoryGrp)
            {
                ...
                field(Inventory; Rec.Inventory)
                {
                    ...
                    begin
                        ...
                        if AdjustInventory.RunModal() in [ACTION::LookupOK, ACTION::OK] then Rec.Get(Rec."No.");
                        CurrPage.Update()
                    end;
                }
            ...
            group("Prices & Sales")
            {
                ...
                field("Price Includes VAT"; Rec."Price Includes VAT")
                {
                    ...
                    trigger OnValidate()
                    begin
                        if Rec."Price Includes VAT" = xRec."Price Includes VAT" then exit;
                    end;
                }
            ...
            group(Availability)
            {
                ...
                group(ItemAvailabilityBy)
                {
                    ...
                    action(Lot)
                    {
                        ...
                        RunPageLink = "No." = field("No."), "Location Filter" = field("Location Filter"), "Variant Filter" = field("Variant Filter");
    ...
    begin
        ...
        if CRMIntegrationEnabled then begin
            ...
            if Rec."No." <> xRec."No." then CRMIntegrationManagement.SendResultNotification(Rec);
        end;
    ...
    begin
        ...
        if CurrentClientType = ClientType::ODataV4 then EnableControls();
        OnAfterOnOpenPage();
    end;
    ...
    begin
        ...
        if IsNonInventoriable then Rec."Stockout Warning" := Rec."Stockout Warning"::No;
    ...
    begin
        if GuiAllowed then if Rec."No." = '' then if DocumentNoVisibility.ItemNoSeriesIsDefault then NewMode := true;
    end;
    ...
    var
        ...
        planningpar: Record "Planning Parameters";
    ...
    begin
        PlanningGetParam.SetPlanningParameters(planningpar);
        // (Rec."Reordering Policy"::"Fixed Reorder Qty.", Rec."Include Inventory",
    ...
    begin
        ...
        if TempSalesPriceAndLineDiscBuff.ItemHasLines(Rec) then SpecialPricesAndDiscountsTxt := ViewExistingTxt;
        SpecialPurchPricesAndDiscountsTxt := CreateNewTxt;
        if TempPurchPriceLineDiscBuff.ItemHasLines(Rec) then SpecialPurchPricesAndDiscountsTxt := ViewExistingTxt;
    end;

    local procedure UpdateSpecialPriceListsTxt(PriceType: Enum "Price Type")
    begin
        if PriceType in [PriceType::Any, PriceType::Sale] then SalesPriceListsText := GetPriceActionText(PriceType::Sale);
        if PriceType in [PriceType::Any, PriceType::Purchase] then PurchPriceListsText := GetPriceActionText(PriceType::Purchase);
    end;
    ...
    begin
        ...
        if PriceListLine.IsEmpty() then exit(CreateNewTxt);
    ...
    begin
        ...
        else if ItemTemplMgt.TemplatesAreNotEmpty() then begin
            CurrPage.Close;
            exit;
        end;
        if ApplicationAreaMgmtFacade.IsFoundationEnabled then if (Item."No." = '') and InventorySetup.Get then Rec.Validate("Costing Method", InventorySetup."Default Costing Method");
    end;
