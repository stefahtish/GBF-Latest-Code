report 50220 "P9A Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = './P9AReport.rdl';
    UseRequestPage = true;
    ApplicationArea = All;
    ...
    dataset
    {
        ...
        {
            //DataItemTableView = sorting("No.") WHERE("Employee Type" = FILTER(<> Trustee));
            ...
            dataitem("Payroll PeriodX"; "Payroll PeriodX")
            {
                ...
                column(NonCashBenefit; NonCashBenefit)
                {
                }
                ...
                column(MortgageRelief_Control202; MortgageRelief)
                {
                }
                ...
                begin
                    ...
                    */
                    ...
                    if Earn.Find('-') then begin
                        ...
                        AssMatrix.SetRange(AssMatrix."Payroll Period", "Starting Date");
                        AssMatrix.SetRange(Code, Earn.Code);
                        if AssMatrix.Find('-') then MortgageRelief := AssMatrix.Amount;
                    end;
                    // Get Other non-cah benefits
                    Earn.Reset;
                    Earn.SetRange("Non-Cash Benefit", true);
                    Earn.SetFilter(Earn."Earning Type", '=%1', Earn."Earning Type"::"Normal Earning");
                    if Earn.Find('-') then begin
                        AssMatrix.Reset;
                        AssMatrix.SetRange(AssMatrix.Type, AssMatrix.Type::Payment);
                        AssMatrix.SetRange(AssMatrix."Employee No", Employee."No.");
                        AssMatrix.SetRange(AssMatrix."Payroll Period", "Starting Date");
                        AssMatrix.SetRange(Code, Earn.Code);
                        if AssMatrix.Find('-') then NonCashBenefit := AssMatrix.Amount;
                    end;
                    ...
                    TotalMortgageRelief := TotalMortgageRelief + MortgageRelief;
            ...
            begin
                GetDefaults(Period);
                if (Period = 0) then Error('Please specify the correct period on the option of the request form');
    ...
    var
        ...
        Mortgage_ReliefCaptionLbl: Label 'Mortgage Relief';
    ...
    procedure GetDefaults(Year: Integer)
    begin
        Period := Year;
        //StopDate:=DMY2DATE(31,12, (DATE2DMY(StopDate,3)));
        StringDate := DMY2DATE(1, 1, Year);
