page 50253 "Pending Purchase Order List"
{
    Caption = 'Pending Purchase Order List';
    ...
    SourceTableView = WHERE("Document Type" = CONST(Order), "Old LPO" = CONST(false), Status = FILTER("Pending Approval"), "LPO Closed" = CONST(false));
    ApplicationArea = All;
            ...
            group("Request Approval")
            {
                ...
                action(SendApprovalRequest)
                {
                    ...
                    begin
                        ...
                        if (Rec."LPO Type" = Rec."LPO Type"::Consumables) and not Rec.Replenishment then begin
                            if Confirm('%1 has a type %2 and will be committed to the budget. Continue?', false, Rec."No.", Rec."LPO Type") = true then
                                Committment.LPOCommittment(Rec, ErrorMsg)
                            else
                                exit;
                        end;
                        if Rec."Cancel Comments" <> '' then begin
                            Rec."Cancel Comments" := '';
                            Rec."Cancelled By" := '';
                            Rec.Modify;
                        end;
