report 50124 "Phys. Stock Evaluation Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = './PhysStockEvaluationReport.rdlc';
    ...
    dataset
    {
        dataitem(PageLoop; "Integer")
        {
            ...
            dataitem("Item Journal Batch"; "Item Journal Batch")
            {
                ...
                dataitem("Item Journal Line"; "Item Journal Line")
                {
                    ...
                    dataitem(ItemTrackingSpecification; "Integer")
                    {
                        ...
                        begin
                            ...
                            if ReservEntryBuffer.IsEmpty then CurrReport.Break;
                ...
                trigger OnAfterGetRecord()
                begin
                    if ItemJournalTemplate.Get("Journal Template Name") then if ItemJournalTemplate.Type <> ItemJournalTemplate.Type::"Phys. Inventory" then CurrReport.Skip;
                end;
            }
        }
    }
            ...
            repeat
                if ItemJnlLine."Bin Code" <> '' then begin
                    if WhseTrckg(ItemJnlLine."Item No.") then
                        PickSNLotFromWhseEntry(ItemJnlLine."Item No.", ItemJnlLine."Variant Code", ItemJnlLine."Location Code", ItemJnlLine."Bin Code", ItemJnlLine."Unit of Measure Code")
                    else begin
    ...
    begin
        ...
        if ItemLedgEntry.FindSet then
            repeat /* CreateReservEntry(ItemJnlLine, ItemLedgEntry."Remaining Quantity",
        ...
        if WhseEntry.FindSet then
            repeat
                if (WhseEntry."Lot No." <> '') and (WhseEntry."Serial No." <> '') then
                    CreateReservEntry(ItemJnlLine, WhseEntry."Qty. (Base)", WhseEntry."Serial No.", WhseEntry."Lot No.", ItemTrackg::"Lot and Serial No.")
                else begin
                    if WhseEntry."Lot No." <> '' then CreateReservEntry(ItemJnlLine, WhseEntry."Qty. (Base)", WhseEntry."Serial No.", WhseEntry."Lot No.", ItemTrackg::"Lot No.");
                    if WhseEntry."Serial No." <> '' then CreateReservEntry(ItemJnlLine, WhseEntry."Qty. (Base)", WhseEntry."Serial No.", WhseEntry."Lot No.", ItemTrackg::"Serial No.");
                end;
            until WhseEntry.Next = 0;
    end;

    local procedure CreateReservEntry(ItemJournalLine: Record "Item Journal Line"; Qty: Decimal; SerialNo: Code[20]; LotNo: Code[20]; ItemTracking: enum "Item Tracking Entry Type")
    var
        FoundRec: Boolean;
    begin
        ReservEntryBuffer.SetCurrentKey("Item No.", "Variant Code", "Location Code", "Item Tracking", "Reservation Status", "Lot No.", "Serial No.");
        ...
        if ReservEntryBuffer.FindSet then begin
            repeat
                if (ReservEntryBuffer."Source Ref. No." = ItemJournalLine."Line No.") and (ReservEntryBuffer."Source ID" = ItemJournalLine."Journal Template Name") and (ReservEntryBuffer."Source Batch Name" = ItemJournalLine."Journal Batch Name") then FoundRec := true;
    ...
    begin
        ...
        repeat
            if (ItemNo <> ItemJnlLine1."Item No.") or (VariantCode <> ItemJnlLine1."Variant Code") or (LocationCode <> ItemJnlLine1."Location Code") then
                NewGroup := true
            else
                ItemJnlLine2 := ItemJnlLine1;
