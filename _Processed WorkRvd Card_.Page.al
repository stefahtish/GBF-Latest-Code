page 51011 "Processed WorkRvd Card"
{
    DelayedInsert = true;
    DeleteAllowed = true;
    Editable = true;
    ...
    actions
    {
        area(processing)
        {
            ...
            action("Mark As Processed")
            {
                ...
                trigger OnAction()
                begin
                    if Confirm('Post', true) then Contract_WorkRv.Reset;
                    Contract_WorkRv.SetRange("Contract No.", Rec."Contract No.");
                    Contract_WorkRv.SetRange("IPC No.", Rec."IPC No.");
                    if Contract_WorkRv.Find('-') then begin
                        workP.Init;
                        workP.TransferFields(Contract_WorkRv);
                        workP.Insert;
                        Contract_WorkRv.Delete;
                    end
                end;
            }
        }
