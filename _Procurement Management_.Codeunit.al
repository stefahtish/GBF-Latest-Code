codeunit 50126 "Procurement Management"
{
    ...
    var Text001: Label 'Please contact administrator for posting setup';
        ...
        if Confirm('Are you sure you want to cancel LPO No.%1', false, LPONo) = true then begin
            ...
            if PurchaseHeader.Find('-')then begin
                ...
                if PurchaseLine.Find('-')then begin
                    repeat PurchaseLine.Committment:=false;
                        PurchaseLine.Modify;
 ...
 ProcurementRequestLine.Reset;
    ...
    begin
        ...
        if SupplierSelection.Find('-')then repeat PreQualifiedList.Reset;
            ...
            if SupplierSelection.Find('-')then begin
                repeat //Mail
                    ...
                    if RecordLink.Find('-')then begin
                        i:=1;
                        repeat AdditionalFilename[i]:=RecordLink.URL1;
    ...
    begin
        ...
        if SupplierSelection.Invited then begin
            ...
            if PreQualifiedList.Find('-')then begin
                if PreQualifiedList."Vendor No" = '' then if Confirm('The system will create Vendor %1 Do you want to continue?', true, SupplierSelection."Supplier Name")then begin
                ...
                if ProcurementRequestLine.Find('-')then repeat PurchaseLine.Init;
    ...
    begin
        ...
        if ProcurementRequest.FindFirst()then begin
            ...
            if BiddersSelect.Find('-')then begin
                ...
                if ProspectiveList.Find('-')then begin
                    if ProspectiveList."Vendor No" = '' then if Confirm('The system will create Vendor %1 Do you want to continue?', true, ProspectiveList.Name)then begin
        ...
        if IRLine.Find('-')then begin
            ...
            repeat LineNo:=LineNo + 10000;
                ...
                if IRHeader2.Get(IRLine."Document No.")then begin
                    ...
                    if not ReqLine.Find('-')then IRHeader2."GPR Created For All Lines":=true
                    else
    ...
    procedure GetFirstVendor(IR: Record "Internal Request Header")Vendor: Code[30]var
        IRLine: Record "Internal Request Line";
    begin
        IRLine.Reset;
        IRLine.SetRange("Document No.", IR."No.");
    ...
    var
        ...
        Accepted: Integer;
    begin
        repeat InspectionLines.SetRange("Order No", PuchLine."Document No.");
            ...
            if InspectionLines.Find('-')then begin
                ...
                if LinesWithDecision = NoOfLines then begin
                    ...
                    if InspectionLines.Find('-')then begin
                        ...
                        if LinesWithDecision = Accepted then PuchLine."Inspection Decisions":=PuchLine."Inspection Decisions"::Accept
                        else
                        begin
    ...
    begin
        ...
        if InspectionLines.Find('-')then repeat PurchLines.Reset();
    ...
    procedure GetOpenEvaluationPeriod(): Code[20]var
        EvalPeriod: Record "Vendor Evaluation Periods";
    begin
        EvalPeriod.Reset;
        EvalPeriod.SetRange(Active, true);
        if EvalPeriod.FindFirst then exit(EvalPeriod.Code)
        else
            ...
            if Approved = Total then begin
                ...
                if EvalLines.Find('-')then begin
                    ...
                    if EvalLines.Find('-')then begin
                        ...
                        //Message('Total mandatory submitted, %1', Submitted);
                        if Passed = Passmark then repeat EvalHeader.Stage:=EvalHeader.Stage::Prices;
                                EvalHeader.Modify();
                            until EvalHeader.Next() = 0
                        else
                            repeat EvalHeader.Stage:=EvalHeader.Stage::Archived;
                                EvalHeader.Modify();
                            until EvalHeader.Next() = 0;
                    end;
                end;
    ...
    begin
        ...
        if TenderAmountLines.Find('-')then repeat //Window.UPDATE(1,"Line No");
                case TenderAmountLines."Scope Type" of TenderAmountLines."Scope Type"::"Objective Heading", TenderAmountLines."Scope Type"::"Objective Heading End": i:=0;
                TenderAmountLines."Scope Type"::"Sub-Heading", TenderAmountLines."Scope Type"::"Sub-Heading End": i:=1;
                TenderAmountLines."Scope Type"::"Scope Parameter": i:=2;
                end;
    ...
    begin
        ...
        if ScopeLines.Find('-')then repeat //Window.UPDATE(1,"Line No");
                case ScopeLines."Scope Type" of ScopeLines."Scope Type"::"Objective Heading", ScopeLines."Scope Type"::"Objective Heading End": i:=0;
                ScopeLines."Scope Type"::"Sub-Heading", ScopeLines."Scope Type"::"Sub-Heading End": i:=1;
                ScopeLines."Scope Type"::"Scope Parameter": i:=2;
                end;
    ...
    begin
        ...
        if SuppTender.FindSet()then begin
            repeat IRLine.Reset;
                ...
                if IRLine.FindFirst()then begin
                    if ProspectiveSuppliers.Get(IRLine."Vendor No")then begin
                        if ProspectiveSuppliers."Vendor No" = '' then Error('Please create a related vendor for prospective supplier %1.', ProspectiveSuppliers."No.")
                        else
                            VendNo:=ProspectiveSuppliers."Vendor No";
                    end;
                    //Message(VendNo);
    ...
    begin
        ...
        if tenderCommittees.FindFirst()then begin
            case tenderCommittees."Committee Type" of tenderCommittees."Committee Type"::Evaluation: begin
                Clear(Instr);
                ...
                if CommitteeMemberRec.FindSet()then repeat Clear(Recipient);
                        EmployeeRec.Reset();
            ...
            tenderCommittees."Committee Type"::Opening: begin
                Clear(Instr);
                TenderCommittee.Reset();
                TenderCommittee.SetRange("Appointment No", DocNo);
    ...
    begin
        ...
        IRLine.SetRange("Document No.", IR."No.");
        if IRLine.Find('-')then repeat "Line No":="Line No" + 1000;
                JobJnlLine.Init;
    ...
    begin
        ...
        IRLine.SetRange("Document No.", IR."No.");
        if IRLine.Find('-')then begin
            repeat if((IRLine.Quantity = 0) and (IRLine."Qty. to Issue" = 0))then Error('Both Quantity Requested and Quantity to issue can not be 0');
                //Check Availability
        ...
        if ItemLedgerEntry.FindSet then begin
            ...
            if IR.Modify(true)then begin
                ...
                IRLine.SetRange("Document No.", IR."No.");
                if IRLine.FindSet then begin
                    repeat IRLine."Quantity Issued":=IRLine."Qty. to Issue";
                        IRLine."Quantity Received":=IRLine."Qty. to Issue";
                        IRLine.Posted:=true;
                        IRLine.Modify(true);
                    until IRLine.Next = 0;
    ...
    begin
        ...
        if IRLine.Find('-')then begin
            repeat IRLineReqNo:=IRLine."Requisition No.";
    ...
    begin
        ...
        if AckHeader.FindFirst then error('Acknowledgement already exists for this order %1', POrder."No.")
        else
        begin //Insert header
            AckHeader.init;
            AckHeader."No.":='';
    ...
    begin
        ...
        if not InspectionCommittee.FindFirst then error(TenderCommitteeNotExistErr2, POrder."Procurement Method", POrder."Tender/Quotation ref no")
        else
        begin
        ...
        if BiddersSelect.Find('-')then begin
            repeat TendersApplied.Reset();
                ...
                if TendersApplied.Find('-')then begin
                    ...
                    if not TenderCommittee.FindFirst then error(TenderCommitteeNotExistErr, TenderNo)
                    else
                    begin
                        //Create evaluation for each committee member
                        CommitteeMembers.Reset();
                        ...
                        if CommitteeMembers.Find('-')then repeat //Insert header
                                SupplierEval.init;
                                SupplierEval."No.":='';
    ...
    begin
        ...
        if TendersApplied.FindSet then begin
            repeat TenderCommittee.reset;
                TenderCommittee.SetRange("Tender/Quotation No", QuoteNo);
                TenderCommittee.SetRange(Status, TenderCommittee.Status::Released);
                TenderCommittee.SetRange("Committee Type", TenderCommittee."Committee Type"::Evaluation);
                if not TenderCommittee.FindFirst then error(TenderCommitteeNotExistErr, QuoteNo)
                else
                begin
                    //Message('ONE');
                            ...
                            if TenderRec.Get(QuoteNo)then begin
                                ...
                                if ScoreSetup.Find('-')then begin
                                    if ScoreSetup."Score Criteria" = ScoreSetup."Score Criteria"::Score then begin
                                        ...
                                        if lvSCoreLines.FindSet()then repeat if(lvSCoreLines.Passmark = 0) or (lvSCoreLines."Maximum Score" = 0)then Error('Please make sure that the evaluation lines have both the passmark and maximum marks to be attained');
    ...
    begin
        ...
        BiddersSelect.Reset();
        BiddersSelect.SetRange("Reference No.", EOINO);
        if BiddersSelect.Find('-')then repeat TendersApplied.SetRange("Tender No.", EOINO);
                TendersApplied.SetRange("Prospect No.", BiddersSelect."Supplier");
                TendersApplied.SetRange("Sent for Evaluation", false);
                if TendersApplied.Find('-')then begin
                    TenderCommittee.reset;
                    ...
                    TenderCommittee.SetRange("Committee Type", TenderCommittee."Committee Type"::Evaluation);
                    TenderCommittee.SetRange(Status, TenderCommittee.Status::Released);
                    if not TenderCommittee.FindFirst then error(EOICommitteeNotExistErr, EOINO)
                    else
                    begin
                        //Create evaluation for each committee member
                        CommitteeMembers.Reset();
    ...
    begin
        ...
        BiddersSelect.Reset();
        BiddersSelect.SetRange("Reference No.", RFPNo);
        if BiddersSelect.Find('-')then repeat TendersApplied.SetRange("Tender No.", RFPNo);
                TendersApplied.SetRange("Prospect No.", BiddersSelect."Supplier");
                TendersApplied.SetRange("Sent for Evaluation", false);
                if TendersApplied.Find('-')then begin
                    TenderCommittee.reset;
                    ...
                    TenderCommittee.SetRange("Committee Type", TenderCommittee."Committee Type"::Evaluation);
                    TenderCommittee.SetRange(Status, TenderCommittee.Status::Released);
                    if not TenderCommittee.FindFirst then error(TenderCommitteeNotExistErr, RFPNo)
                    else
                    begin
                        //Create evaluation for each committee member
                        CommitteeMembers.Reset();
    ...
    begin
        ...
        if TendersApplied.Find('-')then begin
            repeat TenderCommittee.reset;
                ...
                TenderCommittee.SetRange(Status, TenderCommittee.Status::Released);
                if not TenderCommittee.FindFirst then error(TenderCommitteeNotExistErr, TendersApplied."Tender No.")
                else
                begin
                    //Create evaluation for each committee member
                    CommitteeMembers.Reset();
    ...
    local procedure CheckifFullyordered(var IR: Record "Internal Request Header")FullyOrdered: Boolean var
    ...
    local procedure GetLineNo(DocNo: Code[20]): Integer var
        LineNo: Integer;
        PurchaseHeader: Record "Purchase Header";
        PurchaseLine: Record "Purchase Line";
    begin
        ...
        if PurchaseLine.FindLast then LineNo:=LineNo + 1000
        else
            LineNo:=1000;
    end;
    local procedure GetQuoteDate(QuoteNo: Code[20]): Date var
        ProcurementRequest: Record "Procurement Request";
    begin
        ProcurementRequest.Reset;
        ProcurementRequest.SetRange("No.", QuoteNo);
    ...
    local procedure GetQuoteDueDate(QuoteNo: Code[20]): Date var
        ProcurementRequest: Record "Procurement Request";
    begin
        ProcurementRequest.Reset;
        ProcurementRequest.SetRange("No.", QuoteNo);
    ...
    begin
        ...
        if PurchHeader.FindFirst()then begin
            ...
            if PurchLine.Find('-')then repeat PurchLine.TestField("No.");
                    Clear(Receipient);
    ...
    begin
        ...
        if FuelAllocLines.Find('-')then repeat FuelLedgerEntries.Init();
    ...
    begin
        ...
        if PurchLines.FindSet()then repeat PurchLines.TestField("Gen. Bus. Posting Group");
    ...
    begin
        ...
        if TenderEvaluationLine.FindSet()then begin
            repeat ProspectiveSupplier.Reset();
    ...
    begin
        ...
        if CommitteeMembers.FindSet()then begin
            repeat if EmployeeRec.Get(CommitteeMembers."Employee No")then begin
                    EmployeeRec.TestField("Company E-Mail");
