page 51206 "Project Risk Register"
{
    PageType = List;
    SourceTable = "Risk Register";
    SourceTableView = WHERE(Type = FILTER(Project), Archive = FILTER(false));
    ApplicationArea = All;
    ...
    actions
    {
        area(processing)
        {
            ...
            action("Send to Department Register")
            {
                Image = DocumentEdit;
                Promoted = true;
                PromotedCategory = Process;
                ...
                trigger OnAction()
                begin
                    AuditMgt.SendToDepartmentRegister(Rec);
                end;
            }
            action("Archive Risk")
            {
    ...
    var
        ...
        UserNotFoundErr: Label 'The user %1 does not exist in the User Setup!';
        RiskNote: BigText;
        RiskNotesText: Text;
        Instr: InStream;
        OutStr: OutStream;
