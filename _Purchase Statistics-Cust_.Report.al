report 50154 "Purchase Statistics-Cust"
{
    DefaultLayout = RDLC;
    RDLCLayout = './PurchaseStatistics-Cust.rdlc';
        ...
        dataitem(Vendor; Vendor)
        {
            ...
            begin
                ...
                for i := 1 to 5 do begin
                    ...
                    CalcFields("Purchases (LCY)", "Inv. Discounts (LCY)", "Pmt. Discounts (LCY)", "Pmt. Disc. Tolerance (LCY)", "Pmt. Tolerance (LCY)");
                    ...
                    if (VendPurchLCY[i] <> 0) or (VendInvDiscAmountLCY[i] <> 0) or (VendPaymentDiscLCY[i] <> 0) then PrintVend := true;
                end;
                if not PrintVend then CurrReport.Skip();
            end;
        }
    }
    requestpage
    {
        ...
        trigger OnOpenPage()
        begin
            if PeriodStartDate[2] = 0D then PeriodStartDate[2] := WorkDate;
    ...
    begin
        ...
        for i := 2 to 4 do PeriodStartDate[i + 1] := CalcDate(PeriodLength, PeriodStartDate[i]);
    ...
    var
        ...
        VendPurchLCY_1_CaptionLbl: Label 'Purchases (KES)';
        VendInvDiscAmountLCY_1_CaptionLbl: Label 'Inv. Discounts (KES)';
        VendPaymentDiscLCY_1_CaptionLbl: Label 'Pmt. Discounts (KES)';
        VendPaymentDiscTolLcy_1_CaptionLbl: Label 'Pmt. Disc. Tolerance (KES)';
        VendPaymentTolLcy_1_CaptionLbl: Label 'Payment Tolerance (KES)';
        TotalCaptionLbl: Label 'Total';
        VendPurchLCY_1__Control40CaptionLbl: Label 'Purchases (KES)';
        VendInvDiscAmountLCY_1__Control46CaptionLbl: Label 'Inv. Discounts (KES)';
        VendPaymentDiscLCY_1__Control52CaptionLbl: Label 'Pmt. Discounts (KES)';
        VendPaymentDiscTolLcy_1__Control77CaptionLbl: Label 'Pmt. Disc. Tolerance (KES)';
        VendPaymentTolLcy_1__Control78CaptionLbl: Label 'Payment Tolerance (KES)';
