page 50316 "Q Commitee Creation Card"
{
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Appointment letter Ref No."; Rec."Appointment letter Ref No.")
                {
                    ApplicationArea = All;
                }
            }
            ...
            part("Ext Committee Members"; "External Committee")
            {
                Caption = 'Extenal Committee Members';
                Editable = DocEditable;
                SubPageLink = "Appointment No" = field("Appointment No");
            }
        }
            ...
            action("Evaluation Scores")
            {
                ...
                begin
                    ...
                    if CommiteeRec.FindFirst() then begin
                        ScoreSetup.Reset();
                        ScoreSetup.SetRange("Procurement Ref No.", CommiteeRec."Tender/Quotation No");
                        If ScoreSetup.FindFirst() then
                            if ScoreSetup."Score Criteria" = ScoreSetup."Score Criteria"::Score then begin
                                Report.Run(Report::"Tender Evaluation Scores", true, false, CommiteeRec);
                            end
                            else if ScoreSetup."Score Criteria" = ScoreSetup."Score Criteria"::"Yes/No" then begin
                                Report.Run(Report::"Other Tender Evaluation Score", true, false, CommiteeRec);
                            end
                            else
                                Report.Run(Report::"Tender Evaluation Yes/No", true, false, CommiteeRec);
                    end;
                end;
            }
        }
    ...
    begin
        Rec."Procurement Method" := Rec."Procurement Method"::Quotation;
