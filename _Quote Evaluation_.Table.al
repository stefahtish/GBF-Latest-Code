table 50417 "Quote Evaluation"
{
    fields
    {
        ...
        field(6; Quantity; Integer)
        {
            ...
            begin
                ...
                "Amount Inclusive VAT":=Quantity * "Unit Amount";
            end;
        }
            ...
            /* GetHeader();
                if Awarded then begin
                    if not Suggested then begin
                        Message('You are required to insert a comment');
        ...
        field(41; Comments; Text[250])
        {
            DataClassification = ToBeClassified;

            trigger OnValidate()
            var
                EvalHeader: Record "Quote Evaluation Header";
            begin
                EvalHeader.Reset();
                EvalHeader.SetRange("Quote No", "Quote No");
                if EvalHeader.FindFirst()then begin
                    EvalHeader."Award Comment":=Comments;
                    EvalHeader.Modify();
                end;
            end;
        }
    ...
    Direction: Text[30];
    QuoteHeader: Record "Quote Evaluation Header";
    ...
    procedure GetHeader()
    begin
        if QuoteHeader.get("Quote No")then;
    end;
}
