report 50503 "RFQ Report"
{
    UsageCategory = ReportsAndAnalysis;
    Caption = 'RFQ Evaluation Report';
    ApplicationArea = All;
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/Report 51521635 RFQ Report.rdl';
        ...
        dataitem("Procurement Request"; "Procurement Request")
        {
                ...
                dataitem("Supplier Evaluation Header"; "Supplier Evaluation Header")
                {
                    ...
                    dataitem("Supplier Evaluation Document"; "Supplier Evaluation Document")
                    {
                        ...
                        begin
                            ...
                            Clear(ResposiveNess);
                            DocumentsRec.Reset();
                            DocumentsRec.SetRange("Document Code", "Supplier Evaluation Document"."Document Code");
                            DocumentsRec.SetRange("Supplier Code", "Supplier Evaluation Document"."Supplier Code");
                            DocumentsRec.SetRange("Tender No.", "Supplier Evaluation Document"."Tender No.");
                            DocumentsRec.SetRange(Stage, DocumentsRec.Stage::Preliminary);
                            DocumentsRec.SetRange(DocumentsRec.Submitted, false);
                            if DocumentsRec.FindFirst()then ResposiveNess:='NR'
                            else
                                ResposiveNess:='R';
                        end;
                    }
            ...
            dataitem("Procurement Request Lines"; "Procurement Request Lines")
            {
                ...
                dataitem("Tender Evaluation Line"; "Tender Evaluation Line")
                {
                    ...
                    column(TotalCost; "Amount Inclusive VAT")
                    {
                    }
                    column(LineNo; "Line No")
                    {
                    }
                    column(RecmondedAmount; MinAmount)
                    {
                    }
                    trigger OnAfterGetRecord()
                    begin
                        ...
                        //Procedure to Insert Recommended Amount:
                        SuggestRecommendedAmount();
                    end;
                }
            }
    ...
    MinAmount: Decimal;
    //Procedure to Suggest Recommended Amount:
    procedure SuggestRecommendedAmount()
    var
        TenderEvalLine: Record "Tender Evaluation Line";
    begin
        TenderEvalLine.Reset();
        TenderEvalLine.SetRange("Quote No", "Tender Evaluation Line"."Quote No");
        TenderEvalLine.SetRange("Line No", "Tender Evaluation Line"."Line No");
        TenderEvalLine.SetRange(Type, "Tender Evaluation Line".Type);
        TenderEvalLine.SetRange(Awarded, true);
        TenderEvalLine.SetCurrentKey("Amount Inclusive VAT");
        if TenderEvalLine.FindFirst()then begin
            MinAmount:=TenderEvalLine."Amount Inclusive VAT";
        end;
    end;
}
