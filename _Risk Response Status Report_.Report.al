report 50383 "Risk Response Status Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = './RiskResponseStatusReport.rdl';
    ApplicationArea = All;
    ...
    dataset
    {
        dataitem("Risk Header"; "Risk Header")
        {
            ...
            dataitem("Risk Line"; "Risk Line")
            {
                ...
                column(Timelines_RiskLine; GetTimelines("Risk Line"."ME Line No."))
                {
                }
    ...
    local procedure GetTimelines(LineNo: Integer): Date
    var
        RiskLine: Record "Risk Line";
    begin
        RiskLine.Reset;
        RiskLine.SetRange("Document No.", "Risk Header"."No.");
        RiskLine.SetRange(Type, RiskLine.Type::Response);
        RiskLine.SetRange("Line No.", LineNo);
        if RiskLine.FindFirst then exit("Risk Line".Timelines);
    end;
}
