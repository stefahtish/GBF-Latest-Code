page 51384 "Supplier Evaluation Card3"
{
        ...
        area(Processing)
        {
            action(Submit)
            {
                ...
                begin
                    ...
                    // ProcMgmt.GetPreliminaryEvaluationDecision(Rec."No.");
                    //else
                    if not Confirm('Are you sure you want to submit your scores?') then
                        exit
                    else begin
                        Rec.Submitted := true;
    ...
    //Procedure to Update All the Documents in Preliminary Stage:
    procedure UpdatePreliminaryStage(SupplierEvaluationheader: Record "Supplier Evaluation Header")
    var
        SupplierDocs: Record "Supplier Evaluation Document";
        SupplierEvalHeader: Record "Supplier Evaluation Header";
    begin
        SupplierEvalHeader.Reset();
        SupplierEvalHeader.SetRange("No.", SupplierEvaluationheader."No.");
        if SupplierEvalHeader.FindFirst() then begin
            SupplierDocs.Reset();
            SupplierDocs.SetRange("Quote No.", SupplierEvalHeader."No.");
            SupplierDocs.SetRange("Tender No.", SupplierEvalHeader."Quote No");
            if SupplierDocs.FindSet() then begin
                repeat
                    SupplierDocs.Stage := SupplierDocs.Stage::Preliminary;
                    SupplierDocs.Modify();
                until SupplierDocs.Next() = 0;
            end;
        end;
    end;
}
