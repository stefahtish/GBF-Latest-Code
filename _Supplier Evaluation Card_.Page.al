page 50816 "Supplier Evaluation Card"
{
    DeleteAllowed = false;
    PageType = Card;
    ...
    actions
    {
        area(Processing)
        {
            action(Submit)
            {
                Caption = 'Submit Preliminary';
                Image = SendTo;
                Promoted = true;
                PromotedCategory = Process;
                PromotedIsBig = true;
                Visible = Rec.Stage = Rec.Stage::Preliminary;
                ...
                begin
                    if not Confirm('Are you sure you want to submit the document for preliminary analysis?', false) then
                        exit
                    else begin
                        if Rec.Stage = Rec.Stage::Preliminary then ProcMgmt.GetPreliminaryEvaluationDecision(Rec."No.");
                    end;
                end;
            }
