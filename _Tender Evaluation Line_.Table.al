table 50420 "Tender Evaluation Line"
{
    fields
    {
        ...
        field(2; "Vendor No"; Code[20])
        {
            ...
            trigger OnValidate()
            begin
                if ProspectiveRec.Get("Vendor No")then "Vendor Name":=ProspectiveRec.Name;
        ...
        field(8; Amount; Decimal)
        {
            ...
            trigger OnValidate()
            begin
                if(amount <> 0) and ("VAT Code" <> '')then CalcVATAmount();
            // CALCFIELDS("Committed Amount");
            //
            // IF Amount>"Committed Amount" THEN
        ...
        field(9; Awarded; Boolean)
        {
            ...
            begin
                ...
                if Awarded then begin
                    ...
                    /* AwardComment.SetTableView(QuoteHeader);
                        IF AwardComment.RunModal = ACTION::OK then begin
                            QuoteHeader.Modify();
        ...
        {
            CalcFormula = Min("Quote Evaluation".Amount WHERE("Quote No"=FIELD("Quote No"), Type=FIELD(Type), "No."=FIELD("No.")));
            Editable = false;
            FieldClass = FlowField;
        }
        ...
        field(41; "Negotiated Unit Amount"; Decimal)
        {
            trigger OnValidate()
            begin
                if "Negotiated Unit Amount" <> 0 then begin
                    Amount:="Negotiated Unit Amount" * Quantity;
                    Validate(Amount);
                end;
            end;
        }
        field(42; "Negotiated Amount"; Decimal)
        {
            trigger OnValidate()
            begin
                if "Negotiated Amount" <> 0 then begin
                    "Unit Amount":="Negotiated Amount" / Quantity;
                    Validate(Amount);
                end;
            end;
        }
        field(43; Comment; Text[2000])
        {
            DataClassification = ToBeClassified;
        }
        field(44; Specification2; Blob)
        {
            Caption = 'Specification';
            DataClassification = ToBeClassified;
        }
        field(45; Specifications; Text[100])
        {
            DataClassification = ToBeClassified;
        }
    }
    keys
    {
        key(Key1; "Quote No", "Vendor No", "No.", "Line No")
        {
            Clustered = true;
        }
    }
    ...
    TenderHeader: Record "Tender Evaluation Header";
    AwardComment: page "Award Comment";
    ...
    procedure GetHeader()
    begin
        if TenderHeader.get("Quote No")then;
    end;
}
