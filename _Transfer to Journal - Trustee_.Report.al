report 50265 "Transfer to Journal - Trustee"
{
    ProcessingOnly = true;
    ApplicationArea = All;
    ...
    dataset
    {
        ...
        {
            DataItemTableView = WHERE("Employment Type" = FILTER(Trustee), Status = FILTER(Active));
            ...
            begin
                        ...
                        if Earn."Basic Salary Code" = true then begin
                            ...
                            if PostingGroup.Find('-') then begin
                                ...
                                if AssignMatrix.Find('-') then begin
                                    ...
                                    GenJnline.Description := Earn.Description + ' ' + Format(Datefilter, 0, '<day> <month text> <year4>') + ' ' + Employees."No." + '-' + Employees."First Name";
                    ...
                    repeat
                        ...
                        if AssignMatrix.Find('-') then begin
                            ...
                            if Ded."Customer Entry" = false then begin
                                if not ded."PAYE Code" then begin
                                    GenJnline.Init;
                                    LineNumber := LineNumber + 10;
                                    //GenJnline."Journal Template Name":='GENERAL';
                                    GenJnline."Journal Template Name" := HRSetup."Payroll Journal Template";
                                    GenJnline."Journal Batch Name" := JName;
                                    GenJnline."Line No." := GenJnline."Line No." + 10000;
                                    Ded.TestField("Account No.");
                                    GenJnline."Account Type" := Ded."Account Type";
                                    GenJnline."Account No." := Ded."Account No.";
                                    GenJnline.Validate("Account No.");
                                    GenJnline."Posting Date" := Payday;
                                    GenJnline.Description := Ded.Description + ' ' + Format(Datefilter, 0, '<day> <month text> <year4>') + ' ' + Employees."No." + '-' + Employees."First Name";
                                    GenJnline."Shortcut Dimension 1 Code" := Employees."Global Dimension 1 Code";
                                    GenJnline."Shortcut Dimension 2 Code" := Employees."Global Dimension 2 Code";
                                    GenJnline."Currency Code" := Employees."Currency Code";
                                    GenJnline.Validate("Currency Code");
                                    ...
                                    PostingGroup.Get(Employees."Posting Group");
                                    ...
                                    GenJnline."Account No." := PostingGroup."PAYE Account";
                                    GenJnline.Validate("Account No.");
                                    GenJnline."Posting Date" := Payday;
                                    GenJnline.Description := Ded.Description + ' ' + Format(Datefilter, 0, '<day> <month text> <year4>') + ' ' + Employees."No." + '-' + Employees."First Name";
                                    GenJnline."Shortcut Dimension 1 Code" := Employees."Global Dimension 1 Code";
                                    ...
                                    if LoanProductType.FindFirst then begin
                                        if Ded2.Get(LoanProductType."Interest Deduction Code") then begin
                                            if AssignMatrix."Loan Interest" <> 0 then begin
                                                ...
                                                GenJnline."Journal Batch Name" := JName;
                                                GenJnline."Line No." := GenJnline."Line No." + 10000;
                                                GenJnline."Account Type" := Ded2."Account Type";
                                                Employees.TestField("Debtor Code");
                                                GenJnline."Account No." := Ded2."Account No.";
                                                GenJnline."Posting Date" := Payday;
                                                GenJnline.Description := Ded2.Description + ' ' + Format(Datefilter, 0, '<day> <month text> <year4>') + ' ' + Employees."No." + '-' + Employees."First Name";
                                                GenJnline."Shortcut Dimension 1 Code" := Employees."Global Dimension 1 Code";
                                                GenJnline."Shortcut Dimension 2 Code" := Employees."Global Dimension 2 Code";
                                                GenJnline."Currency Code" := Employees."Currency Code";
                                                GenJnline.Validate("Currency Code");
                ...
                if PostingGroup.Find('-') then begin
                    ...
                    GenJnline."Posting Date" := Payday;
                    GenJnline.Description := 'Net Pay' + ' ' + Format(Datefilter, 0, '<day> <month text> <year4>') + ' ' + Employees."No." + '-' + Employees."First Name";
                    GenJnline."Shortcut Dimension 1 Code" := Employees."Global Dimension 1 Code";
                    GenJnline.Validate("Shortcut Dimension 1 Code");
                    GenJnline."Shortcut Dimension 2 Code" := Employees."Global Dimension 2 Code";
                    GenJnline.Validate("Shortcut Dimension 2 Code");
        ...
        layout
        {
            area(content)
            {
                field("Payroll Period"; Datefilter)
                {
                    ...
                    begin
                        PayrollPeriod.Reset;
                        if PAGE.RunModal(Page::"Pay Period Trustees", PayrollPeriod) = ACTION::LookupOK then Datefilter := PayrollPeriod."Starting Date";
                    end;
                }
    ...
    begin
        HRSetup.Get;
        if Confirm('Journal successfully transfered to %1 Journal %2 Batch. Do you want to open?', false, HRSetup."Payroll Journal Template", JName) then begin
            if GenJnlBatch.get(HRSetup."Payroll Journal Template", JName) then GenJnlMgt.TemplateSelectionFromBatch(GenJnlBatch);
        end;
    end;
    ...
    var
        ...
        Payday: Date;
        TotalBasic: Decimal;
        PayrollPeriod: Record "Payroll Period Trustees";
    ...
    procedure GetPayPeriod(var PayPeriods: Record "Payroll Period Trustees")
    begin
