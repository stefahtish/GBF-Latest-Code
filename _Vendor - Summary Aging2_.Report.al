report 50428 "Vendor - Summary Aging2"
{
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem(Vendor; Vendor)
        {
            ...
            column(InVendBalanceDueLCY_8; InVendBalanceDueLCY[9])
            {
            AutoFormatType = 1;
            }
            ...
            dataitem("Integer"; "Integer")
            {
                ...
                begin
                    ...
                    if not Currency2."Vendor Ledg. Entries in Filter" then CurrReport.Skip();
                    PrintLine:=false;
                    ...
                    for i:=1 to 14 do begin
                        ...
                        if VendBalanceDue[i] <> 0 then PrintLine:=true;
                        LineTotalVendAmountDue:=LineTotalVendAmountDue + VendBalanceDue[i];
                    end;
                end;
                trigger OnPreDataItem()
                begin
                    Integer.SetRange(Number, 1, 15);
                    if PrintAmountsInLCY or not PrintLine then CurrReport.Break();
                    Currency2.Reset();
                    Currency2.SetRange("Vendor Filter", Vendor."No.");
                    Vendor.CopyFilter("Currency Filter", Currency2.Code);
                    if not PrintLine then CurrReport.Break();
                    if(Vendor.GetFilter("Global Dimension 1 Filter") <> '') or (Vendor.GetFilter("Global Dimension 2 Filter") <> '')then begin
            ...
            begin
                ...
                for i:=1 to 14 do begin
                    ...
                    if PrintAmountsInLCY then InVendBalanceDueLCY[i]+=DtldVendLedgEntry."Amount (LCY)"
                    else
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(Enddate; Enddate)
                    {
                        ...
                        Caption = 'End Date';
                        NotBlank = true;
                        ToolTip = 'Specifies the End of the period covered by the report that lists payables owed to each vendor.';
                    }
    ...
    begin
        ...
        PeriodStartDate[2]:=Enddate;
        VendFilter:=FormatDocument.GetRecordFiltersWithCaptions(Vendor);
        for i:=3 to 14 do PeriodStartDate[i]:=CalcDate(PeriodLength, PeriodStartDate[i - 1]);
    ...
    StartDate: Date;
    Enddate: Date;
    ErrorEmptyStartDate: Label 'Period Start date must be specified';
    ErrorEmptyEndDate: Label 'Peirod End Date mus be specified';
    ...
    LineTotalVendAmountDue_Control36CaptionLbl: Label 'Total';
