report 50162 "Vendor - Top 10 List-Cust"
{
    DefaultLayout = RDLC;
    RDLCLayout = './VendorTop10List-Cust.rdlc';
    ...
    dataset
    {
        dataitem(Vendor; Vendor)
        {
            ...
            begin
                ...
                if ("Purchases (LCY)" = 0) and ("Balance (LCY)" = 0) then CurrReport.Skip();
                VendAmount.Init();
        ...
        dataitem("Integer"; "Integer")
        {
            ...
            column(Vendor__Purchases__LCY__Caption; VendorPurchCaptionLCY)
            {
            }
            column(Vendor__Balance__LCY__Caption; VendorBalanceCaptionLCY)
            {
            }
            ...
            begin
                if Number = 1 then begin
                    if not VendAmount.Find('-') then CurrReport.Break();
                end
                else if VendAmount.Next = 0 then CurrReport.Break();
                VendAmount."Amount (LCY)" := -VendAmount."Amount (LCY)";
                Vendor.Get(VendAmount."Vendor No.");
                Vendor.CalcFields("Purchases (LCY)", "Balance (LCY)");
                if MaxAmount = 0 then MaxAmount := VendAmount."Amount (LCY)";
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(Show; ShowType)
                    {
                        ...
                        Caption = 'Show';
                        OptionCaption = 'Purchases (KES),Balance (KES)';
    ...
    var
        ...
        BarText: Text[50];
        i: Integer;
        Text004: Label 'Purchases (KES),Balance (KES)';
        ...
        VendorPurchCaptionLCY: Label 'Purchases (KES)';
        VendorBalanceCaptionLCY: Label 'Balance (KES)';
